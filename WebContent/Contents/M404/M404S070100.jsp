...
<script type="text/javascript">
    ...
    function fn_MainInfo_List() {
        ...
        {
            type: "POST",
            url: "<%=Config.this_SERVER_path%>/Contents/Business/M404/S404S070100.jsp", 
            data: "From=" + from + "&To=" + to  + "&jspPage=" + "<%=JSPpage%>",
            beforeSend: function () {
                $("#MainInfo_List_contents").children().remove();
            },
...
// 		alert(vOrderNo);
    	...
    	modalContentUrl = "<%=Config.this_SERVER_path%>/Contents/Business/M404/S404S070101.jsp?OrderNo=" + vOrderNo 
    			+ "&lotno=" + vLotNo
    			+ "&product_nm=" + vProdNm
                + "&lot_count=" + vLotCount
    	...
    	pop_fn_popUpScr(modalContentUrl, obj.innerText+"(S404S070101)", '750px', '1600px');
     }  
...
//     	pop_fn_popUpScr(modalContentUrl, obj.innerText+"(S101S020121)", '420px', '960px');
		return false;
     }    
...
// 			return false;
// 		}
    	var  modalContentUrl = "<%=Config.this_SERVER_path%>/Contents/Business/M404/S404S070102.jsp?OrderNo=" + vOrderNo 
		+ "&OrderDetail=" + vLotNo
			+ "&jspPage=" + "<%=JSPpage%>" ;
    	pop_fn_popUpScr(modalContentUrl, obj.innerText+"(S404S070102)", '500px', '1600px');
     }
    //부적합등록
...
//     	}
		if(checklist_cd.length < 1){
			$('#alertNote').html("<%=JSPpage%>!!! <BR><BR> 삭제할 제품을 선택하세요!!");
			$('#modalalert').show();
			return;
		}
    	
    	var modalContentUrl;
    	modalContentUrl = "<%=Config.this_SERVER_path%>/Contents/Business/M404/S404S070103.jsp?OrderNo=" + vOrderNo 
			+ "&lotno=" + vLotNo 
			+ "&product_nm=" + vProdNm
...
// 	        + "&checklist_cd=" + checklist_cd
			...
			+ "&jspPage=" + "<%=JSPpage%>"
			+ "&num_gubun=<%=GV_GET_NUM_PREFIX%>";
    	pop_fn_popUpScr(modalContentUrl, obj.innerText+"(S404S070103)", '750px', '1600px');
    ...
    function pop_fn_inspect_result_Comlete(obj) {
    	...
    	var modalContentUrl = "<%=Config.this_SERVER_path%>/Contents/Business/M404/S404S070122.jsp?OrderNo=" + vOrderNo 
		+ "&lotno=" + vLotNo 
		+ "&product_nm=" + vProdNm
		+ "&gubun_code=" + vgubun_code
        + "&lot_count=" + vLotCount
		...
		pop_fn_popUpScr(modalContentUrl, obj.innerText+"(S404S070122)", '750px', '1600px');
		return false;
     }
	...
	<div class="panel-body">
		...
		<div>
			<button data-author="mainBtn" type="button" id="mainButton"
				class="btn btn-primary active" style="width: auto; float: left;">품질관리-초품검사</button>
			<button data-author="insert" type="button" onclick="pop_fn_inspect_result_Insert(this)" id="insert"
				class="btn btn-default" style="width: auto; float: left; margin-left: 30px;">초품검사등록</button>
...
<!-- 				class="btn btn-outline-success" style="width: auto; float: left; margin-left: 3px;">초품검사수정</button> -->
...
<!-- 				class="btn btn-outline-success" style="width: auto; float: left; margin-left: 3px;">부적합등록</button> -->
			<button data-author="delete" type="button" onclick="pop_fn_inspect_result_Delete(this)" id="delete"
				class="btn btn-danger" style="width: auto; float: left; margin-left: 3px;">초품검사삭제</button>
<!-- 	        <button data-author="update" type="button" onclick="pop_fn_inspect_result_Comlete(this)" id="update"  -->
<!-- 	        	class="btn btn-warning"  style="width: auto; float: left;  margin-left:30px;">초품검사완료</button> -->
...
<!-- 			<div class="panel-heading" style="font-size: 16px; font-weight: bold" id="InfoContentTitle"></div> -->
			...
			<div id="tabs">
				<ul >
					<li onclick='fn_DetailInfo_List()'><a id="DetailInfo_List"  href='#SubInfo_List_contents'>초품검사결과</a></li>
