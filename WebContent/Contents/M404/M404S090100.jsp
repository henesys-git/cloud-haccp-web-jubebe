...
<script type="text/javascript">
    ...
    function fn_MainInfo_List() {
        ...
        {
            type: "POST",
            url: "<%=Config.this_SERVER_path%>/Contents/Business/M404/S404S090100.jsp", 
...
//                 $("#MainInfo_List_contents").hide().html(html).fadeIn(100);
	...
	function fn_DetailInfo_List() {     
    	$.ajax(
    	    {
    	        type: "POST",
    	        url: "<%=Config.this_SERVER_path%>/Contents/Business/M404/S404S090110.jsp",
    	        data: "Balju_req_date=" + vBalju_req_date + "&Balju_no=" + vBalju_no  + "&Part_cd=" + vPart_cd  ,
    	        beforeSend: function () {
//     	            //$("#SubInfo_List_contents").children().remove();
    	        },
    	        success: function (html) {
    ...
    //출하수락시험등록
    function pop_fn_accept_test_Insert(obj) {
    	var modalContentUrl;
    	
    	modalContentUrl = "<%=Config.this_SERVER_path%>/Contents/Business/M404/S404S080101.jsp?jspPage=" + "<%=JSPpage%>"  + "&num_gubun=<%=GV_GET_NUM_PREFIX%>";
    	pop_fn_popUpScr(modalContentUrl, obj.innerText+"(S404S080101)", '500px', '1550px');
     }  
    
    //출하수락시험수정
    function pop_fn_accept_test_Update(obj) {
// 		if(vOrderNo.length < 1){
<%-- 			$('#alertNote').html(obj.innerText + " <BR> <%=JSPpage%>!!! <BR><BR> 주문정보를 선택하세요  !!!"); --%>
// 			$('#modalalert').show();
// 			return false;
// 		}
    	var  modalContentUrl = "<%=Config.this_SERVER_path%>/Contents/Business/M404/S404S080102.jsp?OrderNo=" + vOrderNo 
		+ "&OrderDetail=" + vOrderDetailSeq
			+ "&jspPage=" + "<%=JSPpage%>" ;
    	pop_fn_popUpScr(modalContentUrl, obj.innerText+"(S404S080102)", '500px', '1550px');
     }
    
    //출하수락시험삭제
    function pop_fn_accept_test_Delete(obj) {		
...
// 		}
    	var modalContentUrl;
    	modalContentUrl = "<%=Config.this_SERVER_path%>/Contents/Business/M404/S404S080103.jsp?OrderNo=" + vOrderNo 
		+ "&OrderDetail=" + vOrderDetailSeq
		+ "&jspPage=" + "<%=JSPpage%>" ;
    	pop_fn_popUpScr(modalContentUrl, obj.innerText+"(S404S080103)", '500px', '1550px');
     }
	...
	<div class="panel-body">
		...
		<div>
			<button data-author="mainBtn" type="button" id="mainButton"
				class="btn btn-primary active" style="width: auto; float: left;">품질관리-출하(수락)검사</button>
			<button data-author="insert" type="button" onclick="pop_fn_accept_test_Insert(this)" id="insert"
				class="btn btn-default" style="width: auto; float: left; margin-left: 30px;">출하(수락)검사등록</button>
			<button data-author="update" type="button" onclick="pop_fn_accept_test_Update(this)" id="update"
				class="btn btn-outline-success" style="width: auto; float: left; margin-left: 3px;">출하(수락)검사수정</button>
			<button data-author="delete" type="button" onclick="pop_fn_accept_test_Delete(this)" id="delete"
				class="btn btn-danger" style="width: auto; float: left; margin-left: 3px;">출하(수락)검사삭제</button>
<!-- 	        <button data-author="update" type="button" onclick="pop_fn_JumunInfo_Comlete(this)" id="update"  -->
<!-- 	        	class="btn btn-warning"  style="width: auto; float: left;  margin-left:30px;">출하(수락)검사완료</button> -->
		...
		<div class="panel panel-default">
			...
			<div id="tabs">
				<ul >
					<li onclick='fn_DetailInfo_List()'><a id="DetailInfo_List"  href='#SubInfo_List_contents'>출하(수락)검사결과</a></li>
