...
//     	            //$("#SubInfo_List_contents").children().remove();
    ...
    //초도품검사등록
    function pop_fn_first_prod_inspect_Insert(obj) {
    	var modalContentUrl;
    	
    	modalContentUrl = "<%=Config.this_SERVER_path%>/Contents/Business/M404/S404S050101.jsp?jspPage=" + "<%=JSPpage%>"  + "&num_gubun=<%=GV_GET_NUM_PREFIX%>";
    	pop_fn_popUpScr(modalContentUrl, obj.innerText+"(S404S050101)", '500px', '1550px');
     }  
    
    //초도품검사수정
    function pop_fn_first_prod_inspect_Update(obj) {
// 		if(vOrderNo.length < 1){
<%-- 			$('#alertNote').html(obj.innerText + " <BR> <%=JSPpage%>!!! <BR><BR> 주문정보를 선택하세요  !!!"); --%>
// 			$('#modalalert').show();
// 			return false;
// 		}
    	var  modalContentUrl = "<%=Config.this_SERVER_path%>/Contents/Business/M404/S404S050102.jsp?OrderNo=" + vOrderNo 
		+ "&OrderDetail=" + vOrderDetailSeq
			+ "&jspPage=" + "<%=JSPpage%>" ;
    	pop_fn_popUpScr(modalContentUrl, obj.innerText+"(S404S050102)", '500px', '1550px');
     }
    
    //초도품검사삭제
    function pop_fn_first_prod_inspect_Delete(obj) {		
...
// 		}
    	var modalContentUrl;
    	modalContentUrl = "<%=Config.this_SERVER_path%>/Contents/Business/M404/S404S050103.jsp?OrderNo=" + vOrderNo 
		+ "&OrderDetail=" + vOrderDetailSeq
		+ "&jspPage=" + "<%=JSPpage%>" ;
    	pop_fn_popUpScr(modalContentUrl, obj.innerText+"(S404S050103)", '500px', '1550px');
     }
	...
	<div class="panel-body">
		...
		<div>
			<button data-author="mainBtn" type="button" id="mainButton" class="btn btn-primary active" 
				style="width: auto; float: left;">품질관리-초도품검사</button>
			<button data-author="insert" type="button" onclick="pop_fn_first_prod_inspect_Insert(this)" id="insert" class="btn btn-default"
				style="width: auto; float: left; margin-left: 30px;">초도품검사등록</button>
			<button data-author="update" type="button" onclick="pop_fn_first_prod_inspect_Update(this)" id="update" class="btn btn-outline-success"
				style="width: auto; float: left; margin-left: 3px;">초도품검사수정</button>
			<button data-author="delete" type="button" onclick="pop_fn_first_prod_inspect_Delete(this)" id="delete" class="btn btn-danger"
				style="width: auto; float: left; margin-left: 3px;">초도품검사삭제</button>
<!-- 	            <button data-author="update" type="button" onclick="pop_fn_JumunInfo_Comlete(this)" id="update" class="btn btn-warning"  -->
<!-- 	            			style="width: auto; float: left;  margin-left:30px;">초도품검사완료</button> -->
