...
<%
	...
	*/
	...
	String GV_CALLER = "0", GV_PART_GUBUN = "", GV_CUST_CD = "";
...
<script type="text/javascript">
	...
	$(document).ready(function () {
		...
		var custCode = "<%=GV_CUST_CD%>";
		
		$("#custList").prepend("<option value='' selected=true>전체</option>");
		
		$('#custList').val(custCode).prop("selected", true);
		
		$('#custList').attr('disabled', true);
		$('#partgubun_mid2').attr('disabled', true);
	    ...
	    $('body').on('change', '#partgubun_big', function(e) {
	    	...
	    	$('#partgubun_mid2').attr('disabled', false);
	    	...
	    	vPartgubun_mid = $('#partgubun_mid2').val();
	    	...
	    	$.ajax({
		        ...
		        success: function (html) {
	            	...
	            	$("#partgubun_mid2").prepend("<option value=''>전체</option>"); 
	    ...
	    function displayPartList() {
	        ...
	        $.ajax({
	            type: "POST",
	            url: "<%=Config.this_SERVER_path%>/Contents/CommonView/PartCodeView2.jsp",
	            data: "partgubun_big="+ vPartgubun_big + 
