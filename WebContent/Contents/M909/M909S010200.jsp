...
<script type="text/javascript">
    ...
    function fn_MainInfo_List() {
        ...
        $.ajax({
	        ...
	        data: "BomCode=" + vBomCode + "&prodgubun_big=" + vProdgubun_big + "&prodgubun_mid=" + vProdgubun_mid,
            beforeSend: function () {
                $("#Main_contents").children().remove();
            },
            success: function (html) {
	...
	function fn_DetailInfo_List() {
    	$.ajax({
   	        ...
   	        data: "prod_cd=" + vProd_cd + "&prod_rev_no=" + vProd_rev_no,
   	        success: function (html) {
    ...
    function pop_fn_BomInfo_Insert(obj) {
    	...
    	var bomTable 	= $("#tableS909S010210_body").find("tr");
    	var bomTableTr 	= bomTable.eq(0);
    	var bomTableTd 	= bomTableTr.children();	
    	var checkVal 	= bomTableTd.eq(0).attr('class').toString();	
    	
    	console.log(checkVal != 'dataTables_empty');
    	
    	if(checkVal != 'dataTables_empty') {
    		heneSwal.warning('이미 해당 제품의 배합정보가 등록되어 있습니다.');
    		return false;	
    	}
		...
		var title = obj.innerText;
		var heneModal = new HenesysModal(url, 'xlarge', title+"(S909S010211)", footer);
		heneModal.open_modal();
    }
    ...
    function pop_fn_BomInfo_Update(obj) {
    	...
    	if(checkVal == 'dataTables_empty') {
    		heneSwal.warning('배합정보를 먼저 등록해 주세요.');
    		return false;	
    	}
		...
		var title = obj.innerText;
		var heneModal = new HenesysModal(url, 'xlarge', title+"(S909S010212)", footer);
		heneModal.open_modal();
    }
<%-- 	
 	// bom 정보 삭제-old
    function pop_fn_BomInfo_Delete(obj) {
		...
		var title = obj.innerText;
		var heneModal = new HenesysModal(url, 'xlarge', title+"(S909S010213)", footer);
		heneModal.open_modal();
    }
    	 --%>
    function pop_fn_BomInfo_Delete(obj) {
    	...
    	var confirmed = confirm('해당 제품의 배합정보를 삭제하시겠습니까?\n'+
								 '모든 이력의 배합정보가 삭제됩니다.');
    	...
    	if(confirmed){
    		...
    		dataJson.prod_cd     = vProd_cd;
    		dataJson.prod_rev_no = vProd_rev_no;
    		...
    		$.ajax({
		        ...
		        data: {"bomdata" : jsonStr, "pid" : "M909S010100E003"},
		        success: function (rcvData) {
		        	if(rcvData < 0) {
		        		heneSwal.error("배합정보 삭제에 실패했습니다, 다시 시도해주세요");
		        	} else {
		        		heneSwal.success("해당 제품의 배합정보가 삭제되었습니다.");
