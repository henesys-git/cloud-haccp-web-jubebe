...
<%--         makeMenu("<%=htmlsideMenu%>"); --%>
        fn_MainSubMenuSelect("<%=sMenuTitle%>"); //공정정보등록검토
// 		$("#InfoContentTitle").html("공정정보");
...
// 		텍스트를 가져오는 방법  
// 	    
// 	        $("#select_GijongCodee").on("change", function(){
...
// 		tr.attr("style", "background-color:#1c86ee");
    ...
    function fn_clearList() { 
        if ($("#ProcessInfo_contents").children().length > 0) {
            $("#ProcessInfo_contents").children().remove();
        }
        if ($("#ProcessInfo_bom").children().length > 0) {
...
//      var custcode = $("#txt_cust_code").val().toString(); 
        ...
        {
            type: "POST",
            url: "<%=Config.this_SERVER_path%>/Contents/Business/M303/S303S010100.jsp", 
            data: "custcode=" + "" + "&From=" + from + "&To=" + to    + "&JSPpage=" + "<%=JSPpage%>",
            beforeSend: function () {
                $("#ProcessInfo_contents").children().remove();
            },
            success: function (html) {
//                 $("#ProcessInfo_contents").hide().html(html).fadeIn(100);
                $("#ProcessInfo_contents").hide().html(html).fadeIn(50);
            },
	...
	function fn_DetailInfo_List() {     
    	$.ajax(
    	    {
    	        type: "POST",
    	        url: "<%=Config.this_SERVER_path%>/Contents/Business/M303/S303S010110.jsp",
...
<!--                 <div class="panel-heading" style="font-size:16px; font-weight: bold"  id="InfoContentTitle"></div> -->
                <div class="panel-body">
                    ...
                    <div id="ProcessInfo_contents"  style="clear:both; " >
                    </div>
                </div>

                <div id="tabs">
		         	<ul >
	                	<li onclick='fn_DetailInfo_List()'><a id="DetailInfo_List"  href='#SubInfo_List_contents'>주문별생산공정상세</a></li>
