...
// 		tr.attr("style", "background-color:#1c86ee");
    ...
    function fn_MainInfo_List() {
        ...
        {
            type: "POST",
            url: "<%=Config.this_SERVER_path%>/Contents/Business/M303/S303S060100.jsp", 
...
//                 $("#MainInfo_List_contents").hide().html(html).fadeIn(100);
	...
	function fn_DetailInfo_List() {     
    	$.ajax(
    	    {
    	        type: "POST",
    	        url: "<%=Config.this_SERVER_path%>/Contents/Business/M303/S303S060110.jsp",
...
//     	            //$("#SubInfo_List_contents").children().remove();
    ...
    //자주검사등록
    function pop_fn_SelfCheck_Insert(obj) {
    	var modalContentUrl;
    	
    	modalContentUrl = "<%=Config.this_SERVER_path%>/Contents/Business/M303/S303S060101.jsp?jspPage=" + "<%=JSPpage%>"  + "&num_gubun=<%=GV_GET_NUM_PREFIX%>";
    	pop_fn_popUpScr(modalContentUrl, obj.innerText+"(S303S060101)", '500px', '1550px');
     }    
    //자주검사수정
    function pop_fn_SelfCheck_Update(obj) {
// 		if(vOrderNo.length < 1){
<%-- 			$('#alertNote').html(obj.innerText + " <BR> <%=JSPpage%>!!! <BR><BR> 주문정보를 선택하세요  !!!"); --%>
// 			$('#modalalert').show();
// 			return false;
// 		}
    	var  modalContentUrl = "<%=Config.this_SERVER_path%>/Contents/Business/M303/S303S060102.jsp?OrderNo=" + vOrderNo 
		+ "&OrderDetail=" + vOrderDetailSeq
			+ "&jspPage=" + "<%=JSPpage%>" ;
    	pop_fn_popUpScr(modalContentUrl, obj.innerText+"(S303S060102)", '500px', '1550px');
     }
    //자주검사삭제
    function pop_fn_SelfCheck_Delete(obj) {		
...
// 		}
    	var modalContentUrl;
    	modalContentUrl = "<%=Config.this_SERVER_path%>/Contents/Business/M303/S303S060103.jsp?OrderNo=" + vOrderNo 
		+ "&OrderDetail=" + vOrderDetailSeq
		+ "&jspPage=" + "<%=JSPpage%>" ;
    	pop_fn_popUpScr(modalContentUrl, obj.innerText+"(S303S060103)", '500px', '1550px');
     }
        ...
        <div class="panel-body" >
        	...
        	<div>
	            <button data-author="mainBtn" type="button"  id="mainButton"  class="btn btn-primary active"
	            			style="width: auto; float: left; ">발주재고관리-자주검사관리</button>
	            <button data-author="insert" type="button" onclick="pop_fn_SelfCheck_Insert(this)" id="insert" class="btn btn-default" 
	            			style="width: auto;float: left; margin-left:30px;">자주검사등록</button>
	            <button data-author="update" type="button" onclick="pop_fn_SelfCheck_Update(this)" id="update" class="btn btn-outline-success" 
	            			style="width: auto;float: left; margin-left:3px;">자주검사수정</button>
	            <button data-author="delete" type="button" onclick="pop_fn_SelfCheck_Delete(this)" id="delete" class="btn btn-danger" 
	            			style="width: auto; float: left; margin-left:3px;">자주검사삭제</button>
<!-- 	            <button data-author="update" type="button" onclick="pop_fn_JumunInfo_Comlete(this)" id="update" class="btn btn-warning"  -->
<!-- 	            			style="width: auto; float: left;  margin-left:30px;">공정실적완료</button> -->
...
<!-- 	            <label style="width: 100px; clear:both; margin-left:3px;"></label> -->
            ...
            <div class="panel panel-default">
                <!-- Default panel contents -->
                <div class="panel-heading" style="font-size:16px; font-weight: bold"  id="InfoContentTitle">자주검사정보</div>
                <div class="panel-body">
                ...
                <div id="tabs">
		         	<ul >
	                	<li onclick='fn_DetailInfo_List()'><a id="DetailInfo_List"  href='#SubInfo_List_contents'>자주검사</a></li>
