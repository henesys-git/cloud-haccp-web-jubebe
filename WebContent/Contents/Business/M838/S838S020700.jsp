...
<%
    ...
    DoyosaeTableModel TableModel = new DoyosaeTableModel("M838S020700E104", jArray);
 	MakeGridData tData = new MakeGridData(TableModel);
    tData.htmlTable_ID = "TableS838S020700";
...
<script>
    ...
    function clickMainMenu(obj){
		...
		regist_seq_no		= td.eq(2).text().trim();
		seq_no 				= td.eq(3).text().trim();
		occur_date 			= td.eq(4).text().trim();
		waste_nm 			= td.eq(5).text().trim();
		weight 				= td.eq(6).text().trim();
		content 			= td.eq(7).text().trim();
		check_yn 			= td.eq(8).text().trim();
...
<table class='table table-bordered nowrap table-hover' 
	...
	<thead>
		<tr>
		     ...
		     <th style='width:0px; display:none;'>일련번호(발생)</th>
		     <th>발생일자</th>
		     <th>폐기물 명</th>
		     <th>중량(kg)</th>
		     <th>처리내용</th>
