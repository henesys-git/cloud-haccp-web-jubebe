...
*/	
	...
	String GV_INSPECTION_YM="", GV_PAGE_START="" ;

	if(request.getParameter("inspection_ym")== null)
		GV_INSPECTION_YM="";
	else
		GV_INSPECTION_YM = request.getParameter("inspection_ym");	
	...
	jArray.put( "inspection_ym", GV_INSPECTION_YM);
	...
	TableModel = new DoyosaeTableModel("M838S070700E144", jArray);
	int RowCount =TableModel.getRowCount();
	
	String InspectionYm="", Writor="", WriteDate="", Checker="", CheckDate="", Approval="", ApproveDate="";
	if(RowCount>0) {
		InspectionYm=TableModel.getValueAt(0, 8).toString().trim() ;
		Writor=TableModel.getValueAt(0, 8).toString().trim() ;
		WriteDate=TableModel.getValueAt(0, 10).toString().trim() ;
		Checker=TableModel.getValueAt(0, 11).toString().trim() ;
		CheckDate=TableModel.getValueAt(0, 12).toString().trim() ;
	...
	for(int i=0; i<RowCount; i++) {
		DataArray.append("[");
		DataArray.append( "'" + TableModel.getValueAt(i, 0).toString().trim() + "'" + "," ); // inspection_date
		DataArray.append( "'" + TableModel.getValueAt(i, 1).toString().trim() + "'" + "," ); // inspection_instrument_no
		DataArray.append( "'" + TableModel.getValueAt(i, 2).toString().trim() + "'" + "," ); // seolbi_nm
		DataArray.append( "'" + TableModel.getValueAt(i, 3).toString().trim() + "'" + "," ); // standard_value
		DataArray.append( "'" + TableModel.getValueAt(i, 4).toString().trim() + "'" + "," ); // specific_value
		DataArray.append( "'" + TableModel.getValueAt(i, 5).toString().trim() + "'" + "," ); // correction_value
		DataArray.append( "'" + TableModel.getValueAt(i, 6).toString().trim() + "'" + "," ); // inspector
		DataArray.append( "'" + TableModel.getValueAt(i, 7).toString().trim() + "'" + "" ); // bigo
...
<%-- 	var DataGrid1_RowCount = <%=RowCount%> + 1 ; // 표1의 행 개수(체크리스트행 개수+헤드) --%>
	...
	var DataGrid1_Height = HaedText_HeightEnd + (DataGrid1_RowCount * DataGrid1_RowHeight) + 520; // 표1 높이( 상단텍스트 끝 위치 + (행개수 * 행높이) + 하단 기준안내문 공간)
	...
	var HeadText = {
		drawText(ctx, sx, sy, ex, ey) {
			var blank_tab = '    '; // 4칸 공백
			var approval_box_width = 270; //결재박스 너비(30 + 80*3)
			// 헤드텍스트
			if('<%=InspectionYm%>'.length>0) {
				var middle_info = '(' 
						+ parseInt('<%=InspectionYm%>'.substring(4,6)) 
						+ '월) 검사 설비 검정 일지' ;
			} else {
				var middle_info = '( 월) 검사 설비 검정 일지' ;
			}
			...
			if('<%=CheckDate%>'.length>0) {
				...
				bottom_check = ("0" + (date.getMonth() + 1)).slice(-2) + ' / ' + ("0" + date.getDate()).slice(-2) ;
			}
			if('<%=ApproveDate%>'.length>0) {
				var date = new Date('<%=ApproveDate%>');
				bottom_approval = ("0" + (date.getMonth() + 1)).slice(-2) + ' / ' + ("0" + date.getDate()).slice(-2) ;
			}
			ctx_fillText(ctx, ex-(approval_box_width-30)*5/6, ey-15, bottom_write, 'black', vTextStyle, 'center','middle');
			ctx_fillText(ctx, ex-(approval_box_width-30)*3/6, ey-15, bottom_check, 'black', vTextStyle, 'center','middle');
			ctx_fillText(ctx, ex-(approval_box_width-30)*1/6, ey-15, bottom_approval, 'black', vTextStyle, 'center','middle');
		} // HeadText.drawText function end
	} ;
	
	// 표1 정의
	var DataGrid1 = {
		col_head:["점검일자","계측기번호","설비명(숨김)","표준값","지시값","교정값","점검자","비    고"],
		col_head_width:[120,180,0,110,110,110,120,120],
		col_unit:[ "℃","g","%" ], // 표1 양식에 들어갈 단위
		col_data:<%=DataArray%>,
...
//		 					ctx_fillText(ctx, col_data_x_center, col_data_y_center, this.col_data[i][j], 'black', vTextStyle, 'center','middle');
	...
	function fn_Pre_Page() {
		var modalContentUrl  = "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S070700_canvas.jsp"
							 + "?inspection_ym=" + "<%=GV_INSPECTION_YM%>"
