...
// 	    var today = new Date("2019-06-02"); // 특정날짜
	    ...
	    for(var i=0; i<TableInoutCheck_RowCount; i++){
			...
			// 결과값 세팅
			var result_value;
			if(trInput.eq(10).val()=='CHECK'){ // 결과값이 체크박스 형태일때
			...
			dataJson.checklist_cd 	 = trInput.eq(0).val();
			dataJson.cheklist_cd_rev = trInput.eq(1).val();
			dataJson.checklist_seq 	 = trInput.eq(2).val();
			dataJson.item_cd 	 	 = trInput.eq(3).val();
			dataJson.item_seq 	 	 = trInput.eq(4).val();
			dataJson.item_cd_rev 	 = trInput.eq(5).val();
			dataJson.check_note 	 = trInput.eq(6).val();
			dataJson.standard_guide  = trInput.eq(7).val();
			dataJson.standard_value  = trInput.eq(8).val();
			dataJson.check_gubun 	 = trInput.eq(9).val();
	...
	<table class="table" style="width: 100%; margin: 0 auto; align:left">
		...
		<tr>
            ...
            <td style="width: auto;  font-weight: 900; font-size:14px; vertical-align: middle ;text-align:left" colspan="2">
				<input type="text" class="form-control" id="txt_check_object" value='<%=TableModel.getValueAt(0, 23).toString().trim()%>'/>
        ...
        <tr style="background-color: #fff; height: 40px">
            ...
            <td >
				...
				<input type="<%=TableModel.getValueAt(i,11).toString().trim()%>" class="form-control" id="txt_check_value" 
						style="width:100%; vertical-align:middle;" value="<%=TableModel.getValueAt(i,7).toString().trim()%>" >
				</input>
