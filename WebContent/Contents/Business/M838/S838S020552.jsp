...
*/
    ...
    DoyosaeTableModel TableModel2 = new DoyosaeTableModel("M838S020550E154", jArray);
 	MakeGridData tData2 = new MakeGridData(TableModel2);
    tData2.htmlTable_ID = "TableS838S020550";
%>
<style>
...
<script type="text/javascript">
    $(document).ready(function () {
    	...
    	var result = <%= tData2.getDataArray() %>;
		...
		for(var i=0; i<data.length; i++){
			...
			html = "<tr> " +
				   ...
				   "<input type = 'hidden' name = 'place_id_"+i+"' value = '"+data[i][3]+"'/> </td> "+
				    place_datail_s+
				   "<td>"+LUX+"</td>"+
				   "<td class='result'> <input type = 'text'  name = 'result_"+i+"' value = '"+result[i][1]+"' required /> </td>"+
				   "<td class='judge'> <label> 적합 <input type = 'radio'  value = 'Y' name = 'judge_"+i+"' required /> </label> " +
				   "<label> 부적합 <input type = 'radio'  value = 'N' name = 'judge_"+i+"'/> </label> </td>"+
				   "</tr>";

			$("#illu_body").append(html);
			
			$('input:radio[name=judge_'+i+']:input[value="'+result[i][2]+'"]').attr("checked",true);
	...
	function SaveOderInfo() {
		...
		if(flag){
			...
			if(chekrtn) {
				SendTojsp(JSONparam, "M838S020550E102");
			}
