...
<%
	// S838S070101.jsp 원자재 입고검사 대장 등록 (PC)
	...
	TableModel = new DoyosaeTableModel("M838S015250E134", jArray);
...
<script type="text/javascript">
	...
	$(document).ready(function () {
		...
		$("#txt_writor").val("<%=optCode%>"); //로그인한 유저rev	
		$("#txt_writor_rev").val("<%=optCode%>"); //로그인한 유저rev	

		var checklist = $("#ccpvrf_check_table_body tr");
		var trInput = $(checklist[3]).find(":input");
		trInput.eq(19).daterangepicker({
		...
		var trInput = $(checklist[5]).find(":input");
		trInput.eq(19).daterangepicker({
			singleDatePicker : true,
			timePicker : true,
			timePicker24Hour : true,
		...
		var trInput = $(checklist[9]).find(":input");
		trInput.eq(19).daterangepicker({
			singleDatePicker : true,
			timePicker : true,
			timePicker24Hour : true,
		...
		var trInput = $(checklist[11]).find(":input");
		trInput.eq(19).daterangepicker({
			singleDatePicker : true,
			timePicker : true,
			timePicker24Hour : true,
		...
		var trInput = $(checklist[15]).find(":input");
		trInput.eq(19).daterangepicker({
			singleDatePicker : true,
			timePicker : true,
			timePicker24Hour : true,
		...
		var trInput = $(checklist[17]).find(":input");
		trInput.eq(19).daterangepicker({
			singleDatePicker : true,
			timePicker : true,
			timePicker24Hour : true,
	...
	function SaveOderInfo() {
		...
		if ($("#txt_check_date").val().length < 1) { // 입고일 선택안하면 저장 X 
			alert("점검날짜를 선택하세요");
			return;
		}
		...
		for (var i = 0; i < checklist.length; i++) {
			...
			dataJson.writor = "<%=loginID%>";
			dataJson.writor_rev = "<%=optCode%>";
			...
			dataJson.action_worker = '';
			dataJson.confirm_worker = '';
...
<table class="table table-bordered" style="width: 100%; margin: 0 auto; text-align:center; height:450px; overflow-y: auto; border-color: black;" id="import_check_table">
	<thead>
	<tr>
        <td style="border-color: black; width:60%;" colspan="2">점검일자</td>
		<td style="border-color: black; width:40%;">
			<input type="text" class="form-control" id="txt_check_date" readonly />
<!-- 			<input type="hidden" class="form-control" id="txt_write_date"  /> -->
<!-- 			<input type="hidden" class="form-control" id="txt_writor"  /> -->
<!-- 			<input type="hidden" class="form-control" id="txt_writor_rev"  /> -->
<!-- 			<input type="hidden" class="form-control" id="txt_approval"  /> -->
<!-- 			<input type="hidden" class="form-control" id="txt_approval_rev"  /> -->
<!-- 			<input type="hidden" class="form-control" id="txt_action_worker"  /> -->
<!-- 			<input type="hidden" class="form-control" id="txt_confirm_worker"  /> -->
		</td>
	</tr>
	<tr>
        <td style="border-color: black; width:60%;" colspan="2">점검자</td>
		<td style="border-color: black; width:40%;">
            <input type="text" class="form-control" id="txt_checker"  />
		</td>
	</tr>
	<tr>
        <td style="border-color: black; width:60%;" colspan="2">수량</td>
		<td style="border-color: black; width:40%;">
			<input type="text" class="form-control" id="txt_part_cnt" numberOnly />
		</td>
	</tr>
	</thead>
	<tbody id="ccpvrf_check_table_body">
<%
...
%>	        
	<tr>
        <td style="border-color: black; width:30%;">
        	<%=GV_CHECK_GUBUN_MID%>
		...
		<td style="border-color: black; width:40%;">
			...
			<input type="<%=TableModel.getValueAt(i,12).toString().trim()%>"  id="txt_check_value" value="CHECK" 
					style="width:30px; height:30px; vertical-align:middle;" checked="checked">
...
<% }%>
	...
	<thead>
	<tr>
        <td style="border-color: black; width:60%;" colspan="2">한계기준 이탈내용</td>
		<td style="border-color: black; width:40%;">
			<input type="text" class="form-control" id="txt_incong_note"  />
		</td>
	</tr>
	<tr>
        <td style="border-color: black; width:60%;" colspan="2">개선조치 및 결과</td>
		<td style="border-color: black; width:40%;">
