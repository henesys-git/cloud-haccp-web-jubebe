...
/* 		    	{
    ...
    function select_product(obj){
        ...
        .row( trNum )
        .index();
// 	alert(vproduct_update_table_Row_index);
    }
	
	function SetRecvData(){
		DataPars(M838S050200E112,GV_RECV_DATA);  		
	...
	function exmData(){
		...
		"	<input type='text' class='form-control' id='txt_HealthExm" + vproduct_update_table_RowCount + "_cd' style='float:left; width:70%; display:none;' readonly></input>" +
		"	<input type='text' class='form-control' id='txt_HealthExm" + vproduct_update_table_RowCount + "_rev' style='float:left; width:70%; display:none;' readonly></input>" +
		<%for(int i=0; i < RowCount; i++ ){%>
	    	vproduct_update_table.row.add( [
	    		"	<input type='text' class='form-control' id='txt_detail_seq'  style='width:30px;' readonly></input>",
	    		"	<input type='text' class='form-control' id='txt_HealthExm" + <%=i%> + "' style='float:left; width:70%;' value='"+ "<%=targetVector[i].get(0).toString()%>" +"' readonly></input>" +
	    		"	<input type='text' class='form-control' id='txt_HealthExm" + <%=i%> + "_cd' style='float:left; width:70%; display:none;' value='' readonly></input>" +
	    		"	<input type='text' class='form-control' id='txt_HealthExm" + <%=i%> + "_rev' style='float:left; width:70%; display:none;' value='' readonly></input>" +
	    		"	<button type='button' onclick='parent.pop_fn_UserList_View(1, \"txt_HealthExm" + <%=i%> +"\")' id='btn_SearchProd' class='btn btn-info' style='float:left;width:30%;' disabled>검색</button> ",
	    		"	<input type='text' data-date-format='yyyy-mm-dd' class='form-control' id='txt_HealthExmDate" + <%=i%> + "' style='width:100%;' value='"+ "<%=targetVector[i].get(1).toString()%>" +"' disabled></input>",
	    		"	<input type='text' data-date-format='yyyy-mm-dd' class='form-control' id='txt_HealthExmNextDate" + <%=i%> + "' style='width:100%;' value='"+ "<%=targetVector[i].get(2).toString()%>" +"' disabled></input>",
				"	<input type='text' class='form-control' id='txt_bigo" + <%=i%> + "' style='width:105%;' value='"+ "<%=targetVector[i].get(20).toString()%>" +"'></input>",	    		
	    		""
	        ] ).draw();
	    	product_update_table_config();
		<%}%>
	...
	function SaveOrderInfo() {        
		...
		if(vproduct_update_table_RowCount == 0) { 
...
//	         	 alert(bomdata);
    ...
    function fn_plus_body(){
    	vproduct_update_table.row.add( [
    		"	<input type='text' class='form-control' id='txt_detail_seq'  style='width:30px;' readonly></input>",
    		"	<input type='text' class='form-control' id='txt_HealthExm" +vproduct_update_table_RowCount + "' style='float:left; width:70%;' readonly></input>" +
    		"	<input type='text' class='form-control' id='txt_HealthExm" + vproduct_update_table_RowCount + "_cd' style='float:left; width:70%; display:none;' readonly></input>" +
    		"	<input type='text' class='form-control' id='txt_HealthExm" + vproduct_update_table_RowCount + "_rev' style='float:left; width:70%; display:none;' readonly></input>" +
    		"	<button type='button' onclick='parent.pop_fn_UserList_View(2, \"txt_HealthExm" + vproduct_update_table_RowCount +"\")' id='btn_SearchProd' class='btn btn-info' style='float:left;width:30%;'>검색</button> ",
    		"	<input type='text' data-date-format='yyyy-mm-dd' class='form-control' id='txt_HealthExmDate" + vproduct_update_table_RowCount + "' style='width:100%;' ></input>",
    		"	<input type='text' data-date-format='yyyy-mm-dd' class='form-control' id='txt_HealthExmNextDate" + vproduct_update_table_RowCount + "' style='width:100%;' ></input>",
			"	<input type='text' class='form-control' id='bigo" + vproduct_update_table_RowCount + "' style='width:105%;' ></input>",	    		
    		""
        ] ).draw();
	...
	function product_update_table_config(){
        $('#txt_HealthExmDate'+ vproduct_update_table_RowCount).datepicker({
        	format: 'yyyy-mm-dd',
        	autoclose: true,
        	language: 'ko'
        });
        $('#txt_HealthExmNextDate'+ vproduct_update_table_RowCount).datepicker({
        	format: 'yyyy-mm-dd',
        	autoclose: true,
        	language: 'ko'
        });
...
// 		vproduct_update_table.draw();

// 		$("#txt_LOTCount").val(vproduct_update_table_RowCount);

// 	    $('input[id=dateDelevery]').eq(vproduct_update_table_RowCount-1).datepicker({
// 	        	format: 'yyyy-mm-dd',
// 	        	autoclose: true,
// 	        	language: 'ko'
// 	    });	
...
// 	    	trInput.eq(5).val($("input[id=txt_LOTNo]").eq(vproduct_update_table_RowCount-1).val() + "-" + vSerialNo.slice(-4));
...
// 	    	trInput.eq(14).val($("input[id=txt_LOTNo]").eq(vproduct_update_table_RowCount-1).val() + "-" + vSerialNo.slice(-4));
   	...
   	<tr>   
		<td style="width: 47%;">
   			<table class="table " style="width: 100%; margin: 0 auto; align:left">
		        <tr style="background-color: #fff;">
		            ...
		            <td ><input type="text" data-date-format="yyyy-mm-dd" id="writeDate" class="form-control" style="width: 200px; border: solid 1px #cccccc;" value="<%=targetVector[0].get(15).toString()%>" readonly/></td>
				</tr>
				<tr style="background-color: #fff;">
					...
					<td ><input type="text" data-date-format="yyyy-mm-dd" id="checkDate" class="form-control" style="width: 200px; border: solid 1px #cccccc;" value="<%=targetVector[0].get(7).toString()%>" readonly/></td>					
				</tr>
				<tr style="background-color: #fff;">
					...
					<td ><input type="text" data-date-format="yyyy-mm-dd" id="approveDate" class="form-control" style="width: 200px; border: solid 1px #cccccc;"  value="<%=targetVector[0].get(11).toString()%>" readonly/></td>										
				</tr>
				<tr style="background-color: #fff;">
		        ...
		        <tr>
		            <td colspan="4"><textarea class="form-control" id="txt_uniqueness"  style="cols:10;rows:4; resize:none;" ><%=targetVector[0].get(19).toString()%></textarea></td>
		        </tr>
        	</table>
        ...
        <tr style="height: 60px">
            <td align="center" colspan="2">
                <p>
                	<button id="btn_Save"  class="btn btn-info" style="width: 100px" onclick="SaveOrderInfo();">수정</button>
