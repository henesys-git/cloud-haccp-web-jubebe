...
// 	alert(vproduct_input_table_Row_index);
	...
	function SetRecvData(){
		DataPars(M838S050200E111,GV_RECV_DATA);  		
...
//	         	 alert(bomdata);
    ...
    function fn_plus_body(){
    	vproduct_input_table.row.add( [
    		...
    		"	<input type='text' class='form-control' id='txt_HealthExm" + vproduct_input_table_RowCount + "' style='float:left; width:70%;' readonly></input>" +
    		"	<input type='text' class='form-control' id='txt_HealthExm" + vproduct_input_table_RowCount + "_cd' style='float:left; width:70%; display:none;' readonly></input>" +
    		"	<input type='text' class='form-control' id='txt_HealthExm" + vproduct_input_table_RowCount + "_rev' style='float:left; width:70%; display:none;' readonly></input>" +
    		"	<button type='button' onclick='parent.pop_fn_UserList_View(2, \"txt_HealthExm" + vproduct_input_table_RowCount +"\")' id='btn_SearchProd' class='btn btn-info' style='float:left;width:30%;'>검색</button> ",
    		"	<input type='text' data-date-format='yyyy-mm-dd' class='form-control' id='txt_HealthExmDate" + vproduct_input_table_RowCount + "' style='width:100%;' ></input>",
    		"	<input type='text' data-date-format='yyyy-mm-dd' class='form-control' id='txt_HealthExmNextDate" + vproduct_input_table_RowCount + "' style='width:100%;' ></input>",
			"	<input type='text' class='form-control' id='txt_bigo" + vproduct_input_table_RowCount + "' style='width:105%;' ></input>",	    		
    		""
        ] ).draw();
        $('#txt_HealthExmDate'+ vproduct_input_table_RowCount).datepicker({
        	format: 'yyyy-mm-dd',
        	autoclose: true,
        	language: 'ko'
        });
        $('#txt_HealthExmNextDate'+ vproduct_input_table_RowCount).datepicker({
        	format: 'yyyy-mm-dd',
        	autoclose: true,
        	language: 'ko'
        });
...
// 		vproduct_input_table.draw();

// 		$("#txt_LOTCount").val(vproduct_input_table_RowCount);

// 	    $('input[id=dateDelevery]').eq(vproduct_input_table_RowCount-1).datepicker({
// 	        	format: 'yyyy-mm-dd',
// 	        	autoclose: true,
// 	        	language: 'ko'
// 	    });	
	    
	    for(i=0;i<vproduct_input_table_RowCount;i++){
		...
		$("#txt_LOTNo"+(vproduct_input_table_RowCount-1)).on('change', function(){ 
	        vproduct_input_table_info = vproduct_input_table.page.info();
	        vproduct_input_table_RowCount = vproduct_input_table_info.recordsTotal; 
	    		
	        vLotNo = trInput.eq(6).val();
	        
// 	    	trInput.eq(5).val($("input[id=txt_LOTNo]").eq(vproduct_input_table_RowCount-1).val() + "-" + vSerialNo.slice(-4));
	    	trInput.eq(5).val(vLotNo + "-" + vSerialNo.slice(-4));
	    }); 
		$("#txt_LOTCount"+(vproduct_input_table_RowCount-1)).on('change', function(){ 
	        vproduct_input_table_info = vproduct_input_table.page.info();
	        vproduct_input_table_RowCount = vproduct_input_table_info.recordsTotal; 
...
// 	    	trInput.eq(14).val($("input[id=txt_LOTNo]").eq(vproduct_input_table_RowCount-1).val() + "-" + vSerialNo.slice(-4));
   			...
   			<table class="table " style="width: 100%; margin: 0 auto; align:left">
		        <tr style="background-color: #fff;">
					...
					<td style="width: 38%; font-weight: 900; font-size:14px; text-align:left">
						<input type="text" class="form-control" id="txt_writer" style="width: 200px; float:left" readonly />
						<input type="text" class="form-control" id="txt_writer_cd" style="width: 200px; float:left; display:none;" readonly />
						<input type="text" class="form-control" id="txt_writer_rev" style="width: 200px; float:left; display:none;" readonly />
						<button type="button" onclick="parent.pop_fn_UserList_View(2, 'txt_writer')" id="btn_SearchUser" class="btn btn-info" style="float:left"> 검색</button> 
		           	</td>
					<td style=" font-weight: 900; font-size:14px; text-align:left">작성일자</td>
		            <td ><input type="text" data-date-format="yyyy-mm-dd" id="writeDate" class="form-control" style="width: 200px; border: solid 1px #cccccc;"/></td>
				</tr>
				<tr style="background-color: #fff;">
					<td style=" font-weight: 900; font-size:14px; text-align:left">점검자명</td>
					<td style="width: 38%; font-weight: 900; font-size:14px; text-align:left">
						<input type="text" class="form-control" id="txt_checker" style="width: 200px; float:left" readonly />
						<input type="text" class="form-control" id="txt_checker_cd" style="width: 200px; float:left; display:none;" readonly />
						<input type="text" class="form-control" id="txt_checker_rev" style="width: 200px; float:left; display:none;" readonly />
						<button type="button" onclick="parent.pop_fn_UserList_View(2, 'txt_checker')" id="btn_SearchUser" class="btn btn-info" style="float:left"> 검색</button> 
		           	</td>		
					<td style="width: 16%; font-weight: 900; font-size:14px; text-align:left">점검일자</td>
					<td ><input type="text" data-date-format="yyyy-mm-dd" id="checkDate" class="form-control" style="width: 200px; border: solid 1px #cccccc;"/></td>					
				</tr>
				<tr style="background-color: #fff;">
					<td style=" font-weight: 900; font-size:14px; text-align:left">승인자명</td>
					<td style="width: 38%; font-weight: 900; font-size:14px; text-align:left">
						<input type="text" class="form-control" id="txt_approve" style="width: 200px; float:left" readonly />
						<input type="text" class="form-control" id="txt_approve_cd" style="width: 200px; float:left; display:none;" readonly />
						<input type="text" class="form-control" id="txt_approve_rev" style="width: 200px; float:left; display:none;" readonly />
						<button type="button" onclick="parent.pop_fn_UserList_View(2, 'txt_approve')" id="btn_SearchUser" class="btn btn-info" style="float:left"> 검색</button> 
		           	</td>					
					<td style="width: 16%; font-weight: 900; font-size:14px; text-align:left">승인일자</td>
					<td ><input type="text" data-date-format="yyyy-mm-dd" id="approveDate" class="form-control" style="width: 200px; border: solid 1px #cccccc;"/></td>										
				</tr>
				<tr style="background-color: #fff;">
					<td style="width: 16%; font-weight: 900; font-size:14px; text-align:left">해당분기</td>
					<td>
						<input type='radio' name='chk_quat' id='radio_quat1' value='quat1'/>1/4분기<label style='width:40px;'/>
						<input type='radio' name='chk_quat' id='radio_quat2' value='quat2'/>2/4분기<label style='width:40px;'/>
						<input type='radio' name='chk_quat' id='radio_quat3' value='quat3'/>3/4분기<label style='width:40px;'/>
						<input type='radio' name='chk_quat' id='radio_quat4' value='quat4'/>4/4분기<label style='width:40px;'/>						
					</td>					
				</tr>	
		        <tr style="background-color: #fff" >
		        ...
		        <tr>
		            <td colspan="4"><textarea class="form-control" id="txt_uniqueness"  style="cols:10;rows:4; resize:none;" ></textarea></td>
		        </tr>
        	</table>
