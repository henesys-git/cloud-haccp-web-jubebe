...
<%
    ...
    DoyosaeTableModel TableModel = new DoyosaeTableModel("M838S080200E104", jArray);	
 	MakeGridData tData = new MakeGridData(TableModel);
    tData.htmlTable_ID = "TableS838S080200";
%>
<script>
    ...
    function clickMainMenu(obj){
		...
		meeting_date 		= td.eq(2).text().trim();
		seq_no 				= td.eq(3).text().trim();
		meeting_topic 		= td.eq(4).text().trim();
		meeting_detail 		= td.eq(5).text().trim();
		meeting_result  	= td.eq(6).text().trim();
		attendees_cnt  		= td.eq(7).text().trim();
		bigo  		   		= td.eq(8).text().trim();
...
<table class='table table-bordered nowrap table-hover' 
	...
	<thead>
		<tr>
			 ...
			 <th>회의주제</th>
			 <th style='width:0px; display:none;'>회의내용</th>
			 <th style='width:0px; display:none;'>회의결과</th>
			 <th>참석인원</th>
			 <th>비고</th>
			 <th>작성자</th>
		     <th>확인자</th>
		     <th>승인자</th>
		     <th style='width:0px; display:none;'>문서구분</th>
		     <th style='width:0px; display:none;'>파일이름</th>
		     <th style='width:0px; display:none;'>파일경로</th>
		     <th style='width:0px; display:none;'>파일수정이력</th>
