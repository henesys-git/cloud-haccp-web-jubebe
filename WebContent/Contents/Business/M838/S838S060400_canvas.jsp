...
*/	
    ...
    String subcontractor_ceo              = "";
    String subcontractor_headoffice_phone = "";
...
<script type="text/javascript">	
	...
	var SubdataText = {
		...
		drawText(ctx, sx, sy, ex, ey) {
			...
			for(var i = 0; i < col_loc.length-1; i++){ ctx_Line(ctx, sx+cut*col_loc[i], sy, sx+cut*col_loc[i], ey, 'black', 1); }
			
			ctx_fillText(ctx, sx+cut*1-cut/2, text_hgt, sub_col_name[0], 'black', vTextStyleBold, 'center','middle');
			ctx_fillText(ctx, sx+cut*2.5, text_hgt, sub_col_name[1], 'black', vTextStyleBold, 'center','middle');
			ctx_fillText(ctx, sx+cut*5.5, text_hgt, sub_col_name[2], 'black', vTextStyleBold, 'center','middle');
			ctx_fillText(ctx, sx+cut*8, text_hgt, sub_col_name[3], 'black', vTextStyleBold, 'center','middle');
			ctx_fillText(ctx, sx+cut*10, text_hgt, sub_col_name[4], 'black', vTextStyleBold, 'center','middle');
			ctx_fillText(ctx, sx+cut*12, text_hgt, sub_col_name[5], 'black', vTextStyleBold, 'center','middle');
			ctx_fillText(ctx, sx+cut*14, text_hgt, sub_col_name[6], 'black', vTextStyleBold, 'center','middle');
			ctx_fillText(ctx, sx+cut*16, text_hgt, sub_col_name[7], 'black', vTextStyleBold, 'center','middle');
			ctx_fillText(ctx, sx+cut*18.5, text_hgt, sub_col_name[8], 'black', vTextStyleBold, 'center','middle');

			for(var i = 0; i < detail_data.length; i++){
				
				ctx_fillText(ctx, sx+cut*1-cut/2, text_hgt+40*(i+1), i+1, 'black', vTextStyle, 'center','middle');              
				ctx_fillText(ctx, sx+cut*2.5, text_hgt+40*(i+1), detail_data[i][3], 'black', vTextStyle, 'center','middle');   // 0. product_division              
				ctx_fillText(ctx, sx+cut*5.5, text_hgt+40*(i+1), detail_data[i][4], 'black', vTextStyle, 'center','middle');   // 1. subcontractor_name            
				ctx_fillText(ctx, sx+cut*8, text_hgt+40*(i+1), detail_data[i][5], 'black', vTextStyle, 'center','middle');     // 2. subcontractor_ceo             
				ctx_fillText(ctx, sx+cut*10, text_hgt+40*(i+1), detail_data[i][6], 'black', '12px 맑은고딕', 'center','middle');    // 3. subcontractor_headoffice_phone
				ctx_fillText(ctx, sx+cut*12, text_hgt+40*(i+1), detail_data[i][7], 'black', vTextStyle, 'center','middle');    // 4. appraisal_means               
				ctx_fillText(ctx, sx+cut*14, text_hgt+40*(i+1), detail_data[i][8], 'black', '12px 맑은고딕', 'center','middle');    // 5. approve_date                  
				ctx_fillText(ctx, sx+cut*16, text_hgt+40*(i+1), detail_data[i][9], 'black', vTextStyle, 'center','middle');    // 6. approval                         
				ctx_fillText(ctx, sx+cut*18.5, text_hgt+40*(i+1), detail_data[i][11], 'black', vTextStyle, 'center','middle'); // 7. product_bigo                  
