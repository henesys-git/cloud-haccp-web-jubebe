...
*/
	...
	if(request.getParameter("ampm_gubun") != null)
		ampm_gubun = request.getParameter("ampm_gubun");
	
	JSONObject jArray = new JSONObject();
	
 	jArray.put("seq_no", seq_no);
	 	
	DoyosaeTableModel table = new DoyosaeTableModel("M838S020600E154", jArray);
...
<script type="text/javascript">
    $(document).ready(function () {
		...
		new SetSingleDate2("<%=table.getStrValueAt(0,3)%>", "#regist_date", 0);
		$('#regist_date').attr('disabled', true);			
		
		// 수정 전 값 넣기
	 	$('#check_time').val('<%=table.getStrValueAt(0,7)%>');
		
	 	$('#temp_processing1').val('<%=table.getStrValueAt(0,8)%>');
	 	$('#temp_processing2').val('<%=table.getStrValueAt(0,9)%>');
	 	$('#temp_packing').val('<%=table.getStrValueAt(0,10)%>');
	 	$('#temp_prod').val('<%=table.getStrValueAt(0,11)%>');
	 	$('input:radio[name=mixing_hour_yn]:input[value="'+'<%=table.getStrValueAt(0,12)%>'+'"]').attr("checked",true);
	 	$('input:radio[name=clean_status_yn]:input[value="'+'<%=table.getStrValueAt(0,13)%>'+'"]').attr("checked",true);
	 	$('input:radio[name=appearance_yn]:input[value="'+'<%=table.getStrValueAt(0,14)%>'+'"]').attr("checked",true);
	 	$('input:radio[name=packing_hour_yn]:input[value="'+'<%=table.getStrValueAt(0,15)%>'+'"]').attr("checked",true);
	 	$('input:radio[name=packing_status_yn]:input[value="'+'<%=table.getStrValueAt(0,16)%>'+'"]').attr("checked",true);
	 	$('input:radio[name=indication_comply_yn]:input[value="'+'<%=table.getStrValueAt(0,17)%>'+'"]').attr("checked",true);
	 	$('input:radio[name=expiration_date_yn]:input[value="'+'<%=table.getStrValueAt(0,18)%>'+'"]').attr("checked",true);
	 	$('#temp_defrosting').val('<%=table.getStrValueAt(0,19)%>');
	 	$('input:radio[name=defrosting_yn]:input[value="'+'<%=table.getStrValueAt(0,20)%>'+'"]').attr("checked",true);
	 	$('#temp_frosting').val('<%=table.getStrValueAt(0,21)%>');
...
/* 	 	dataJson.unsuit_detail = $('#unsuit_detail').val();
		...
		if(chekrtn) {
			 $.ajax({
			        ...
			        data:  {"bomdata" : JSONparam, "pid" : "M838S020600E112"},
