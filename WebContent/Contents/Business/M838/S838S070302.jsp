...
*/
    ...
    DoyosaeTableModel TableModel = new DoyosaeTableModel("M838S070300E164", jArray);
...
<script type="text/javascript">
    $(document).ready(function () {
		...
		for(var i=0; i<data.length; i++){
			...
			html = "<tr>" +
				   ...
				   '<td> <div class="input-group">'+
								'<input type = "number" name = "standard_value_'+i+'" class = "standard_temp temp" value = '+data[i][5]+' />'+
				   ...
				   "</td>"+
				   '<td> <div class="input-group">'+
				   				"<input type = 'number' name = 'check_value_"+i+"' class = 'temp' value = '"+data[i][6]+"'/> "+
								'<div class="input-group-append">'+
								     '<span class="input-group-text">&#8451;</span>'+
						        '</div>'+
						   '</div>'+
				   "</td>"+
				   "<td> <input type = 'text' class = 'stdev_temp' id = 'stdev_temp_"+i+"' style = 'width: 100%;text-align: center;' value = '"+data[i][7]+"' disabled/> "+"</td>"+
				   ...
				   "<td> <input type = 'text' name = 'improve_action_"+i+"' style= 'width: 100%;' value = '"+data[i][9]+"' /> "+"</td>"+
				   "</tr>";

			$("#illu_body").append(html);
			
			$("input:radio[name='judge_"+i+"'][value='"+data[i][8]+"']").prop("checked", true);
		}
		...
		$(".temp").keyup(function() {
			...
			$("#stdev_temp_"+seq).val(stdev_temp.toFixed(2)); // 편차 값 입력
	...
	function SaveOderInfo() {
		...
		if(flag){
			...
			if(chekrtn) {
				SendTojsp(JSONparam, "M838S070300E102");
			}	
		} 	
