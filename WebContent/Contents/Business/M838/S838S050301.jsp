...
<%
	...
	TableModel = new DoyosaeTableModel("M838S050300E134", jArray);
...
// 	    var today = new Date("2019-06-02"); // 특정날짜
	    ...
	    for(var i=0; i<TableInoutCheck_RowCount; i++){
			...
			dataJson.checker_dept 	 = $("#txt_checker_dept").val();
			dataJson.checker_title 	 = $("#txt_checker_title").val();
			dataJson.checker_main  	 = $("#txt_checker_main").val();
			dataJson.checker_main_rev = $("#txt_checker_main_rev").val();
			dataJson.checker_dept_2 	= "";
			dataJson.checker_title_2 = "";
			dataJson.checker_second 	= "";
			dataJson.checker_second_rev = 0;
			dataJson.confirm_dept  = "";
			dataJson.confirm_title = "";
			dataJson.confirm_user  = "";
			dataJson.confirm_user_rev = 0;
			
			dataJson.cust_cd 	 = $("#txt_cust_cd").val();
			dataJson.cust_cd_rev  = $("#txt_cust_cd_rev").val();
			dataJson.cust_nm 	 = $("#txt_cust_nm").val();
			dataJson.boss_name 	 = $("#txt_boss_name").val();
			dataJson.jongmok 	 = $("#txt_jongmok").val();
			dataJson.bizno 		 = $("#txt_bizno").val();
			dataJson.address 	 = $("#txt_address").val();
			dataJson.telno 		 = $("#txt_telno").val();
			dataJson.check_object = $("#txt_check_object").val();
...
//	         	 alert(bomdata);
	...
	function SetCustName_code(txt_custname, txt_custcode, txt_cust_rev, 
			txt_boss_name, txt_jongmok, txt_bizno, txt_address, txt_telno) {
		$("#txt_cust_nm").val(txt_custname);
		$("#txt_cust_cd").val(txt_custcode);
		$("#txt_cust_cd_rev").val(txt_cust_rev);
		
		$("#txt_boss_name").val(txt_boss_name);
		$("#txt_jongmok").val(txt_jongmok);
		$("#txt_bizno").val(txt_bizno);
	...
	<table class="table" style="width: 100%; margin: 0 auto; align:left">
		<tr>
            ...
            <td style="width: auto;  font-weight: 900; font-size:14px; vertical-align: middle ;text-align:left">
				...
				<input type="hidden" class="form-control" id="txt_cust_cd" />
				<input type="hidden" class="form-control" id="txt_cust_cd_rev" />
				<input type="hidden" class="form-control" id="txt_boss_name" />
				<input type="hidden" class="form-control" id="txt_jongmok" />
				<input type="hidden" class="form-control" id="txt_bizno" />
				<input type="hidden" class="form-control" id="txt_address" />
				<input type="hidden" class="form-control" id="txt_telno" />
           	</td>
		...
		<tr>
            ...
            <td style="width: auto;  font-weight: 900; font-size:14px; vertical-align: middle ;text-align:left" colspan="2">
				<input type="text" class="form-control" id="txt_check_object"  />
				<input type="hidden" class="form-control" id="txt_checker_main" />
				<input type="hidden" class="form-control" id="txt_checker_main_rev" />
				<input type="hidden" class="form-control" id="txt_checker_dept" />
				<input type="hidden" class="form-control" id="txt_checker_title" />
           	</td>
		</tr>
	</table>
	...
	%>	 
        <tr style="background-color: #fff; height: 40px">
            <td>
            	...
            	<input type="hidden" id="txt_check_gubun_mid"  readonly value=''></input>
				<input type="hidden" id="txt_check_gubun_sm"  readonly value=''></input>
            ...
            <td >
				...
				<input type="<%=TableModel.getValueAt(i,12).toString().trim()%>" class="form-control" id="txt_check_value" value="CHECK" 
