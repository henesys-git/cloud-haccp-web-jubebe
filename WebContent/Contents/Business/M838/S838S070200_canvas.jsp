...
<%
/* 
부자재 입고검사 대장 canvas (S838S070200_canvas.jsp)
*/	
...
// 	jArray.put( "check_date_end", GV_CHECK_DATE_END);	
	...
	TableModel = new DoyosaeTableModel("M838S070200E144", jArray);
	...
	CheckGubunMid.append("[" + "'','','','','',");
 	CheckGubunSm.append("[");
	CheckNote.append("[" + "'입 고 일','납품 업체명','품 목 명','규    격','제품수량',");
...
//  		}
	...
	CheckGubunMid.append("'','','결재','결재'" + "]");
 	CheckGubunSm.append("]");
	CheckNote.append("'부적합 내용 / 특이사항','개선조치 내용','작    성 (입고담당)','승    인 (HACCP 팀장)'" + "]");
	...
	if(RowCount>=CheckCount) {
		for(int i=0; i<CheckRow; i++) {
			...
			data_sub.append("[");
			data_sub.append("'" + TableModel.getValueAt(i*CheckCount, 2).toString().trim() + "',"); // 입고일
			...
			data_sub.append("'" + TableModel.getValueAt(i*CheckCount, 5).toString().trim() + "',"); // 규격
...
<script type="text/javascript">	
	...
	var HaedText_HeightEnd = HeadText_HeightStart + 125; // 헤드텍스트 영역 종료 높이
	...
	var DataGrid1_RowCount = <%=CheckCount%> + 9; // 표1의 행 개수
	var DataGrid1_ColWidth = 200; // 표1의 열 너비(최소130)
	var DataGrid1_ColWidth1st = 35; // 표1의 1번째 열 너비
...
<%-- 	var DataGrid1_ColCount = <%=CheckRow%> ; // 표1의 데이터 열 개수 --%>
	...
	var DataGrid1_HeightEnd = DataGrid1_HeightStart + DataGrid1_RowCount * DataGrid1_RowHeight; // 표1 끝나는 위치(시작위치 + 행개수 * 행높이)
	...
	var HeadText = {
		drawText(ctx, sx, sy, ex, ey) {
			var blank_tab = '    '; // 4칸 공백
			var top_info = 'HS-PP-06-B 부자재입고검사대장' ;
			var middle_info = '부자재 입고검사 대장' ;
			var bottom_info1 = '- 점검방법 : 양호 O, 불량  X' ;
			var bottom_info2 = '- 검사성적서 : 내포장재(식육에 닿는것)에 한한다.' ;
			var bottom_info3 = '- 작성자 : 품질관리팀 / - 주기 : 입고시마다' ;
			...
			var bottom_info4 = '- 담당자 : (정) ' + writor_main + blank_tab + '(부) '+ writor_second ;
			var bottom_info_width = 350;
			ctx_fillText(ctx, sx, sy, top_info, 'black', '15px 맑은고딕', 'start','top');
			ctx_fillText(ctx, (sx+ex-bottom_info_width)/2, (sy+ey)/2, middle_info, 'black', 'bold 40px 맑은고딕', 'center','middle');
			ctx_fillText(ctx, ex-bottom_info_width, sy + 25, bottom_info1, 'black', 'bold 15px 맑은고딕', 'start','top');
			ctx_fillText(ctx, ex-bottom_info_width, sy + 50, bottom_info2, 'black', 'bold 15px 맑은고딕', 'start','top');
			ctx_fillText(ctx, ex-bottom_info_width, sy + 75, bottom_info3, 'black', 'bold 15px 맑은고딕', 'start','top');
			ctx_fillText(ctx, ex-bottom_info_width, sy + 100, bottom_info4, 'black', 'bold 15px 맑은고딕', 'start','top');
		}
	} ;
	...
	var DataGrid1 = {
		...
		col_unit:[ "","","","","","□양호/□불량","□양호/□불량","□양호/□불량","□양호/□불량","","□적합/□부적합","","","","" ], // 표1 양식에 들어갈 단위	  
		...
		drawGrid: function(ctx, sx, sy, ex, ey) { // 표1 양식 그리기
			...
			for(i=0; i<this.col_headLv1.length; i++) { // 가로
				...
				if(i==0) { // 입고일(가로열 합치기), 0번째엔 선 그릴 필요없이 글자만 넣는다
				...
				} else if(i==6||i==7||i==8||i==14) { // i==5,6,7,8,13,14 일때만 구분선 그어줌(가로열 나누기)
					ctx_Line(ctx, sx+DataGrid1_ColWidth1st, row_height, ex, row_height, 'black', 1); // 가로선
				...
				} else if(i==5||i==13) { // i==5,6,7,8,13,14 일때만 구분선 그어줌(가로열 나누기)
					ctx_Line(ctx, sx, row_height, ex, row_height, 'black', 1); // 가로선
					ctx_Line(ctx, sx+DataGrid1_ColWidth1st, row_height, sx+DataGrid1_ColWidth1st, row_height+DataGrid1_RowHeight, 'black', 1); // 세로선(1번째)
					if(i==5) var col_head_y_union = col_head_y + DataGrid1_RowHeight*3/2;
					else var col_head_y_union = col_head_y + DataGrid1_RowHeight/2;
					ctx_wrapText(ctx, col_headLv1_x, col_head_y_union, this.col_headLv1[i],
							'black', vTextStyleBold, 'center','middle', DataGrid1_ColWidth1st-10, DataGrid1_RowHeight/2);
					ctx_wrapText_space(ctx, col_headLv2_x, col_head_y, this.col_headLv2[i],
							'black', vTextStyleBold, 'center','middle', DataGrid1_ColWidth2nd-10, DataGrid1_RowHeight/2);
				} else if(i==9) { // 검사성적서 확인(하단 안내문)
					...
					// 하단안내문&구분선
					ctx_Line(ctx, sx, row_height+30, sx+DataGrid1_ColWidth1st+DataGrid1_ColWidth2nd-2, row_height+30, 'black', 1);
					var inspect_doc_info = '징  수  시    :    O';
					ctx_fillText(ctx, col_headLv1_x+(DataGrid1_ColWidth2nd)/2, col_head_y+15, inspect_doc_info, 'black', 'bold 12px 맑은고딕', 'center','middle');
...
// 						col_head_y = row_height + (DataGrid1_RowHeight/2); // 다시 계산
				...
				// 세로선(데이터 부분)
				ctx_Line(ctx, sx+DataGrid1_ColWidth1st+DataGrid1_ColWidth2nd-2, row_height, 
						sx+DataGrid1_ColWidth1st+DataGrid1_ColWidth2nd-2, row_height+DataGrid1_RowHeight, 'black', 1); // 이중선
				ctx_Line(ctx, sx+DataGrid1_ColWidth1st+DataGrid1_ColWidth2nd+2, row_height, 
						sx+DataGrid1_ColWidth1st+DataGrid1_ColWidth2nd+2, row_height+DataGrid1_RowHeight, 'black', 1); // 이중선
				if(i==11) { // 부적합 추가된 줄만큼 세로선긋기
					ctx_Line(ctx, sx+DataGrid1_ColWidth1st+DataGrid1_ColWidth2nd-2, row_height-(DataGrid1_IncongLineCount-2)*(DataGrid1_RowHeight/2), 
							sx+DataGrid1_ColWidth1st+DataGrid1_ColWidth2nd-2, row_height, 'black', 1); // 이중선
					ctx_Line(ctx, sx+DataGrid1_ColWidth1st+DataGrid1_ColWidth2nd+2, row_height-(DataGrid1_IncongLineCount-2)*(DataGrid1_RowHeight/2), 
							sx+DataGrid1_ColWidth1st+DataGrid1_ColWidth2nd+2, row_height, 'black', 1); // 이중선
				} else if(i==12) { // 개선조치 추가된 줄만큼 세로선긋기
					ctx_Line(ctx, sx+DataGrid1_ColWidth1st+DataGrid1_ColWidth2nd-2, row_height-(DataGrid1_ImproveLineCount-2)*(DataGrid1_RowHeight/2), 
							sx+DataGrid1_ColWidth1st+DataGrid1_ColWidth2nd-2, row_height, 'black', 1); // 이중선
					ctx_Line(ctx, sx+DataGrid1_ColWidth1st+DataGrid1_ColWidth2nd+2, row_height-(DataGrid1_ImproveLineCount-2)*(DataGrid1_RowHeight/2), 
							sx+DataGrid1_ColWidth1st+DataGrid1_ColWidth2nd+2, row_height, 'black', 1); // 이중선
				} 
				for(j=0; j<DataGrid1_ColCount; j++) { //세로
					if(j>0) { //첫번째 왼쪽 세로선은 이중선으로 위에서 따로 그음
						ctx_Line(ctx, sx+DataGrid1_ColWidth1st+DataGrid1_ColWidth2nd+(j*DataGrid1_ColWidth), row_height, 
								sx+DataGrid1_ColWidth1st+DataGrid1_ColWidth2nd+(j*DataGrid1_ColWidth), row_height+DataGrid1_RowHeight, 'black', 1);
						if(i==11) { // 부적합 추가된 줄만큼 세로선긋기
							ctx_Line(ctx, sx+DataGrid1_ColWidth1st+DataGrid1_ColWidth2nd+(j*DataGrid1_ColWidth),
									row_height-(DataGrid1_IncongLineCount-2)*(DataGrid1_RowHeight/2), 
									sx+DataGrid1_ColWidth1st+DataGrid1_ColWidth2nd+(j*DataGrid1_ColWidth), row_height, 'black', 1);
						} else if(i==12) { // 개선조치 추가된 줄만큼 세로선긋기
							ctx_Line(ctx, sx+DataGrid1_ColWidth1st+DataGrid1_ColWidth2nd+(j*DataGrid1_ColWidth),
									row_height-(DataGrid1_ImproveLineCount-2)*(DataGrid1_RowHeight/2), 
									sx+DataGrid1_ColWidth1st+DataGrid1_ColWidth2nd+(j*DataGrid1_ColWidth), row_height, 'black', 1);
			...
			for(j=0; j<this.col_data.length; j++) { //세로
				...
				for(i=0; i<dataColCount; i++) { // 가로
					...
					if(i==0) { // 작성일자 => 작성연도,일자,점검시각
						...
						var date_string	= "";
						if( !isNaN(datetime.getTime()) ) { // 날짜데이터 유효성검사
							date_string = datetime.getFullYear() + " . "
										+ ("0" + (datetime.getMonth() + 1)).slice(-2) + " . "
										+ ("0" + datetime.getDate()).slice(-2) + " .";
						}
						ctx_fillText(ctx, col_data_x, col_data_y, date_string, 'black', vTextStyle, 'center','middle');
					} else if(i==5||i==6||i==7||i==8) { // 체크박스 => 양호/불량(i==5,8,9,11)
						var check_data = this.col_data[j][i];
						if(check_data == "Y")
							ctx_fillText(ctx, col_data_x-40, col_data_y-3, "✓", 'black', vTextStyle, 'center','middle');
						else if(check_data == "N")
							ctx_fillText(ctx, col_data_x+12, col_data_y-3, "✓", 'black', vTextStyle, 'center','middle');
					} else if(i==9) { // O,X (i==9)
						var check_data = this.col_data[j][i];
						if(check_data == "Y")
	...
	function fn_Pre_Page() {
		var modalContentUrl  = "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S070200_canvas.jsp"
