...
<%
	...
	String GV_BIZNO = "", GV_REV = "" ;
	
	String[] strColumnHead = {"사업자번호", "개정번호", "회사명", "주소", "전화번호", "대표이름", //0~5
			"업태", "종목", "팩스", "홈페이지", "우편번호", "직인이미지", //6~11
			"로고이미지", "적용시작일자", "HACCP인증번호", 		//12~14
			"이력제OpenAPI사용자아이디", "이력제OpenAPI서비스KEY", //15~16
			"이력제사용여부"};   //17 
...
String param = GV_BIZNO + "|"  + GV_REV ;
	...
	System.out.println(targetSulbiVector.capacity());
%>
 
<script type="text/javascript">
	var vSealImageFileName = "";
	var vLogoImageFileName = "";
	var checkValue = '<%=targetSulbiVector.get(17).toString().trim()%>';
	...
	function SaveOderInfo() {
		...
		if ($('#txt_BizNo').val().length < 10) {
			heneSwal.warning("사업자등록번호를 정확히 입력하세요.\n[예:123-12-12345]");
			return;
		}
		if ($('#txt_Cust_nm').val().length < 1) {
			heneSwal.warning("회사 이름을 입력해주세요");
			return;
		}
		...
		if(chekrtn){
			SendTojsp(JSON.stringify(dataJson), "M909S150100E102");
		}
	    ...
	    $.ajax({
	         ...
	         success: function (html) {
	        	 if(html>-1){
			        ...
			        heneSwal.success('회사정보 수정에 성공하였습니다.');
	        	   	parent.fn_MainInfo_List();
	                parent.$("#ReportNote").children().remove();
	                $('#modalReport').modal('hide');
	         	}
	        	else{
	        		heneSwal.error('회사정보 수정에 성공하였습니다.'); 
	        	 }
	         }
	     });		
	}
    ...
    $(document).ready(function () {
	    ...
	    if(vSealImageFileName != "") fn_Set_Image_File(vSealImageFileName, 'SealImageFile'); // 미리보기
	    if(vLogoImageFileName != "") fn_Set_Image_File(vLogoImageFileName, 'LogoImageFile'); // 미리보기
	    ...
	    $("#SealImage").on('change',function(){
	        ...
	        if( image_file_name.indexOf(".") ) { // 파일명 검사(파일이 선택됐는지) 및  확장자 검사(이미지파일-jpg,jpeg,png,gif 맞는지)
	        	...
	        	heneSwal.warning("등록할 이미지 파일을 선택하세요");
		    	return;
	        }
	    });
	    ...
	    $("#LogoImage").on('change',function(){ 
	    	...
	    	if( image_file_name.indexOf(".") != -1 ) { // 파일명 검사(파일이 선택됐는지)
	        	...
	        	if( aExt[1]!='jpg' && aExt[1]!='JPG' && aExt[1]!='JPEG' && aExt[1]!='jpeg' 
	        		...
	        		heneSwal.warning("선택한 파일은 지원하지 않는 파일형식입니다." + "\n" + "jpg, jpeg, png, gif 형식의 파일을 선택하세요");
	    	    	return;
	        	} else {
	    ...
	    $("input[name=txt_hist_yn]").filter("input[value='"+checkValue+"']").attr("checked",true);
    ...
    // 서버/images/Company폴더에 이미지파일 올리기
        ...
        <tr>
        	...
        	<table class="table" style="width: 100%; margin: 0 auto; align:left">
        		<tr style="background-color: #fff; height: 40px">
            		...
            		<!-- <td> </td> -->
            		<td ><input type="text" class="form-control" id="txt_BizNo" 
            		value="" style="width: 200px; float:left" readonly />
           			</td>
        		</tr>
				<tr style="background-color: #fff; height: 40px">
            		<td style="font-weight:900;">회사명</td>
            		<!-- <td> </td> -->
            		<td ><input type="text" class="form-control" id="txt_Cust_nm" 
  				...
  				<tr style="background-color: #fff; height: 40px">
            		...
            		<!-- <td> </td> -->
            		<td ><input type="text" class="form-control" id="txt_Juso" 
            		readonly value=""style="width: 400px; float:left"  />
            		&nbsp;&nbsp;<input type="button" class="btn btn-info" onclick="sample6_execDaumPostcode()" value="입력"></input>
           			</td>
        		</tr>
         		<tr style="background-color: #fff; height: 40px">
            		<td style="font-weight:900;">우편번호</td>
            		<!-- <td> </td> -->
            		<td ><input type="text" class="form-control" id="txt_Zipno" 
            		readonly value="" style="width: 200px; float:left"  />
           			</td>
        		</tr>
		        <tr style="background-color: #fff; height: 40px">
    		        <td style="font-weight:900;">전화번호</td>
        		    <!-- <td> </td> -->
            		<td ><input type="text" class="form-control" id="txt_TelNo" 
            		value="" style="width: 200px; float:left"  />
           			</td>
        		</tr>
				<tr style="background-color: #fff; height: 40px">
            		...
            		<!-- <td> </td> -->
            		<td ><input type="text" class="form-control" id="txt_BossName" 
            		value="" style="width: 200px; float:left"  />
           			</td>
		        </tr>
        		<tr style="background-color: #fff; height: 40px">
            		...
            		<!-- <td> </td> -->
            		<td ><input type="text" class="form-control" id="txt_Uptae" 
            		value="" style="width: 200px; float:left"  />
           			</td>
        		</tr>
        		<tr style="background-color: #fff; height: 40px">
            		...
            		<!-- <td> </td> -->
		            <td ><input type="text" class="form-control" id="txt_Jongmok" 
		            value="" style="width: 200px; float:left"  />
		           	</td>
        		</tr>
 		        <tr style="background-color: #fff; height: 40px">
		            ...
		            <!-- <td> </td> -->
    		        <td ><input type="text" class="form-control" id="txt_Fax" 
    		        value=""style="width: 200px; float:left"  />
		           	</td>
        		</tr>
		        <tr style="background-color: #fff; height: 40px">
		           ...
		           <!--  <td> </td> -->
		            <td ><input type="text" class="form-control" id="txt_HomePage" 
		            value="" style="width: 200px; float:left"  />
		           	</td>
		        </tr>
        		<tr style="background-color: #fff; height: 40px">
        		...
        		<tr style="background-color: #fff; height: 40px">
            		...
            		value="" style="width: 220px; border: solid 1px #cccccc;"/>
        		   	</td>
        		</tr>
        		<tr style="background-color: #fff; height: 40px">
            		<td style="font-weight:900;">변경사유</td>
            		<!-- <td> </td> -->
        		    <td ><input type="text" class="form-control" id="txt_modify_reason" 
        ...
        <tr>
   			<td  style="width: 30%">
        		...
        		<tr style="background-color: #fff; height: 40px">
            		...
            		<!-- <td> </td> -->
            		<td ><input type="text" class="form-control" id="txt_haccp_No" 
            		value="" style="width: 200px; float:left"  />
           			</td>
        		</tr>
				<tr style="background-color: #fff; height: 40px">
        		...
        		<tr style="background-color: #fff; height: 40px">
            		...
            		<!-- <td> </td> -->
            		<td ><input type="text" class="form-control" id="txt_kape_openapi_apikey" 
            		value="" style="width: 200px; float:left"  />
           			</td>
        		</tr>
   				</table>
