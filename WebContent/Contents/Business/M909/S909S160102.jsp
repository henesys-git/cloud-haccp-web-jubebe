...
<%
    ...
    // 데이터를 가져온다.
    Vector CCP_Upgrad_Target_Vector = (Vector)(TableModel.getVector().get(0));

    for( int i = 0 ; i < 9 ; i++ )
    {
    	System.out.println( "CCP_Upgrad_Target_Vector.get(" + i + ") : " + CCP_Upgrad_Target_Vector.get(i).toString().trim() );    	
    }
    
    String CCP_yn = TableModel.getValueAt(0, 6).toString().trim(); 
    String GET_auto_yn = TableModel.getValueAt(0, 7).toString().trim();
    String RECORD_yn = TableModel.getValueAt(0, 8).toString().trim();
    String MONITOR_yn = TableModel.getValueAt(0, 9).toString().trim();
...
//  웹소켓 통신을 위해서 필요한 변수들 ---끝	
    ...
    $(document).ready(function () {
    	$("#txt_CCPType").val("<%=CCP_Upgrad_Target_Vector.get(4).toString()%>");
		...
		$('input[name=rdo_RECORD_yn][value=<%=RECORD_yn%>]').prop("checked", true).change();
		$('input[name=rdo_MONITOR_yn][value=<%=MONITOR_yn%>]').prop("checked", true).change();
	...
	/* function SetRecvData(){
		DataPars(M909S160100E102, GV_RECV_DATA);  	
 		if(M909S160100E102.retnValue > 0)
 			alert('수정 되었습니다.');
	...
	function SaveOderInfo() {     
		...
		if(chekrtn){
			SendTojsp(JSONparam, "M909S160100E102");
		}
...
<table class="table table-hover" style="width: 100%; margin: 0 auto; align:left">
	<tr style="background-color: #fff; height: 40px">
		...
		<td>
			...
			value = "<%=CCP_Upgrad_Target_Vector.get(0).toString()%>" readonly />
		</td>
	</tr>
	...
	<tr style="background-color: #fff; height: 40px">
		...
		<td>
			...
			value = "<%=CCP_Upgrad_Target_Vector.get(1).toString()%>" readonly />
		</td>
	</tr>
	...
	<tr style="background-color: #fff; height: 40px">
		...
		<td>
			...
			value = "<%=CCP_Upgrad_Target_Vector.get(2).toString()%>" readonly />
		</td>
	</tr>
	...
	<tr style="background-color: #fff; height: 40px">
		...
		<td>
			...
			value = "<%=CCP_Upgrad_Target_Vector.get(3).toString()%>" />
		</td>
	</tr>
	...
	<tr style="background-color: #fff; height: 40px">
		...
		<td>
			...
			value = "<%=CCP_Upgrad_Target_Vector.get(5).toString()%>" />
		</td>
	</tr>
	...
	<tr style="background-color: #fff; height: 40px">
		...
		<td>
			<p style="float: left; margin-top: 6px; margin-bottom: 6px;">
				<input type="radio"  id="rdo_RECORD_yn"  name="rdo_RECORD_yn" value="Y" > Y</input>
	...
	<tr style="background-color: #fff; height: 40px">
		...
		<td>
			<p style="float: left; margin-top: 6px; margin-bottom: 6px;">
				<input type="radio"  id="rdo_MONITOR_yn"  name="rdo_MONITOR_yn" value="Y" > Y</input>
