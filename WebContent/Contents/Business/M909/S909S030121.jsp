...
<%
	...
	String GV_CHECK_CODE="", GV_CHECK_NAME="", GV_CHECK_CODE_MID="",GV_CHECK_NAME_MID="";
	...
	if(request.getParameter("check_code_mid")== null)
		GV_CHECK_CODE_MID="";
	else
		GV_CHECK_CODE_MID = request.getParameter("check_code_mid");
	
	if(request.getParameter("check_name_mid")== null)
		GV_CHECK_NAME_MID="";
	else
		GV_CHECK_NAME_MID = request.getParameter("check_name_mid");
...
//  웹소켓 통신을 위해서 필요한 변수들 ---끝	
	...
	function SaveOderInfo() {
		...
		if ($('#txt_sm_Code_Name').val().length < 1) {
			alert("중분류 명을 입력하세요.");
			return;
		}
   		...
   		var dataJson = new Object(); // jSON Object 선언 
            ...
            dataJson.Check_mid = "<%=GV_CHECK_CODE_MID %>";
            dataJson.Check_mid_name = "<%=GV_CHECK_NAME_MID %>";
            
            dataJson.Check_sm_name = $('#txt_sm_Code_Name').val();

            
            $.ajax({
     	         type: "POST",
     	         url: "<%=Config.this_SERVER_path%>/Contents/Business/M909/S909S030124.jsp", 
     	         data:  "Page=<%=JSPpage%>"+"&Check_sm_name=" + $('#txt_sm_Code_Name').val() + "&Check_big=" + "<%=GV_CHECK_CODE %>" + "&Check_mid=" + "<%=GV_CHECK_CODE_MID %>",	         
...
<!-- <form id="mesfrm"> -->
   <table class="table table-hover" style="width: 100%; margin: 0 auto; align:left">
        ...
        <tr style="background-color: #fff; height: 40px">
            ...
            <td> </td>
            <td >
            <input type="text" class="form-control" id="txt_mid_Code_Name" value="<%=GV_CHECK_NAME_MID %>" style="width: 200px; float:left"  readonly="readonly" />
           	</td>
        </tr>
        <tr style="background-color: #fff; height: 40px">
            <td>소분류 명</td>
            <td> </td>
            <td >
            <input type="text" class="form-control" id="txt_sm_Code_Name" style="width: 200px; float:left"  />
           	</td>
        </tr>
