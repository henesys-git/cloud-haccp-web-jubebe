...
// 	String RightButton[][]	= {{"off", "fn_Chart_View", rightbtnChartShow},{"off", "fn_Doc_Reg()", rightbtnDocSave},{"off", "file_real_name", rightbtnDocShow}};
	...
	String GV_PROD_CD="", GV_ORDER_DETAIL_SEQ="", GV_JSPPAGE="", GV_NUM_GUBUN="";
	
	if(request.getParameter("PartCd")== null)
		GV_PROD_CD = "";
	else
		GV_PROD_CD = request.getParameter("PartCd");
...
// 	String param =GV_ORDERNO+ "|" ;
    ...
    // TableModel = new DoyosaeTableModel("M858S060100E144", strColumnHead, param);
...
//  웹소켓 통신을 위해서 필요한 변수들 ---끝	
	...
	var vTableS858S060120;
	var TableS858S060120_info;
    var TableS858S060120_RowCount;
    var S858S060120_Row_index = -1;    

    var vTableS858S060240;
	var TableS858S060240_info;
    var TableS858S060240_RowCount;
    var S858S060240_Row_index = -1;    
...
// 		 $.ajax({
// 		        type: "POST",
<%-- 	            url: "<%=Config.this_SERVER_path%>/Contents/Business/M858/S858S060120.jsp",  --%>
...
// 	 		});
        ...
        $.ajax({
	        type: "POST",
            url: "<%=Config.this_SERVER_path%>/Contents/Business/M858/S858S060240.jsp", 
	...
	function SaveOderInfo() {        
		...
		TableS858S060240_info = vTableS858S060240.page.info();
        TableS858S060240_RowCount = TableS858S060240_info.recordsTotal;
        ...
        var parmHead= "" 
			...
			+ '<%=GV_NUM_GUBUN%>' 				+ "|" 
			+ $('#txt_prod_cd').val()			+ "|"
			+ $('#txt_prodcd_bunho').val()			+ "|"
			+ '<%=member_key%>' + "|"
			+ "<%=Config.HEADTOKEN %>" 	;

		var parmBody= "" ;
	    for(var i=0; i<TableS858S060240_RowCount; i++){   
	    	parmBody += vTableS858S060240.cell(i , 0).data() + "|"	//0 주문번호
	    	+ vTableS858S060240.cell(i , 1).data() + "|"	//1 출고번호
	    	+ vTableS858S060240.cell(i , 2).data() + "|"	//2 출고일자
	    	+ vTableS858S060240.cell(i , 3).data() + "|"	//3 출고일련번호					
	    	+ vTableS858S060240.cell(i , 4).data() + "|"	//4 출고시간
	    	+ vTableS858S060240.cell(i , 5).data() + "|"	//5 출고담당자
	    	+ vTableS858S060240.cell(i , 6).data() + "|"	//6 제품명
	    	+ vTableS858S060240.cell(i , 7).data() + "|"	//7 제품코드\
	    	+ vTableS858S060240.cell(i , 8).data() + "|"	//8 제품개정번호
	    	+ vTableS858S060240.cell(i , 9).data() + "|"	//9 창고번호
	    	+ vTableS858S060240.cell(i , 10).data() + "|"	//10 렉번호
	    	+ vTableS858S060240.cell(i , 11).data() + "|"	//11 선반번호
	    	+ vTableS858S060240.cell(i , 12).data() + "|"	//12 칸번호
	    	+ vTableS858S060240.cell(i , 13).data() + "|"	//13 출고 전 재고량
	    	+ vTableS858S060240.cell(i , 14).data() + "|"	//14 출고 후 재고량
 	    	+ vTableS858S060240.cell(i , 15).data() + "|"	//15 입출고 수량
			+ '<%=Config.DATATOKEN %>' 	;
	    }
		...
		//E111
		/* SendTojsp(SQL_Param.params, M858S020100E101.UPID);
		 */
	}
...
// 	         	 alert(bomdata);
    ...
    function fn_Update_body(obj){  	
    	if(S858S060240_Row_index == -1){
    		vTableS858S060240.row.add( [
    			$('#txt_ipgo_order').val(),
    			$('#txt_ipgo_no').val(),
    			$('#txt_ipgo_date').val(),
    			++ioSeq_Max,
...
//     			'';
    		...
    		vTableS858S060240.cell( S858S060240_Row_index, 0).data( $('#txt_ipgo_order').val() );
    		vTableS858S060240.cell( S858S060240_Row_index, 1).data( $('#txt_ipgo_no').val() );
			vTableS858S060240.cell( S858S060240_Row_index, 2).data( $('#txt_ipgo_date').val() );
			vTableS858S060240.cell( S858S060240_Row_index, 3).data( $('#txt_io_seqno').val() );
			vTableS858S060240.cell( S858S060240_Row_index, 4).data( $('#txt_io_time').val() );
			vTableS858S060240.cell( S858S060240_Row_index, 5).data( $('#txt_io_user_id').val() );
			vTableS858S060240.cell( S858S060240_Row_index, 6).data( $('#txt_prod_name').val() );
			vTableS858S060240.cell( S858S060240_Row_index, 7).data( $('#txt_prod_cd').val() );
			vTableS858S060240.cell( S858S060240_Row_index, 8).data( $('#txt_prodcd_bunho').val() );
			vTableS858S060240.cell( S858S060240_Row_index, 9).data( $('#txt_store_no').val() );
			vTableS858S060240.cell( S858S060240_Row_index, 10).data( $('#txt_rakes_no').val() );
			vTableS858S060240.cell( S858S060240_Row_index, 11).data( $('#txt_plate_no').val() );
			vTableS858S060240.cell( S858S060240_Row_index, 12).data( $('#txt_colm_no').val() );
			vTableS858S060240.cell( S858S060240_Row_index, 13).data( $('#txt_pre_amt').val() );
			vTableS858S060240.cell( S858S060240_Row_index, 14).data( $('#txt_io_amt').val() );
			vTableS858S060240.cell( S858S060240_Row_index, 15).data( $('#txt_post_amt').val() );

    	}
    	vTableS858S060240.draw();
	    clear_input(); 
    }  
...
// 		$('#txt_balju_inspect_no').val("");	
    ...
    function fn_mius_body(obj){
		...
		console.log(trNum + "==fn_Update_body=" + S858S060240_Row_index); 
    	vTableS858S060240.row(S858S060240_Row_index ).remove().draw();
//     	vTableS858S060240.row(trNum ).remove().draw();

	    TableS858S060240_info = vTableS858S060240.page.info();
	    TableS858S060240_RowCount = TableS858S060240_info.recordsTotal;
	    
		for(var i =0; i<TableS858S060240_RowCount; i++){
		    vTableS858S060240.cell( i, 3 ).data( i+1 );
		}	        
    }
    function sum(obj){
...
<!-- 	            <td style="width:3.5%; font-weight: 900; font-size:14px; text-align:center; vertical-align: middle">Rev.</td> -->
	        ...
	        <tr style="vertical-align: middle">	
	        	...
	        	<td style="text-align:center; vertical-align: middle ;margin: 0 ;">
					...
					<input type="text" class="form-control" maxlength="20"  id="txt_prod_name" readonly></input>					
				</td>		           
				
				<td style="text-align:center; vertical-align: middle ;margin: 0 ;">
					<input type="text" 	 class="form-control" maxlength="20" id="txt_prod_cd" readonly ></input> 
	           	</td>
	            ...
	            <td style="font-weight: 900; font-size:14px; text-align:center; vertical-align: middle ;margin: 0 ;">
	            	<input type="hidden"  id="txt_prodcd_bunho" class="form-control"  readonly/>
...
<!-- 	            </td> -->
<!-- 	            <td style="text-align:center; vertical-align: middle ;margin: 0 ;"> -->
<!-- 	            	<input type="text"  maxlength="5" id="txt_prod_amt" class="form-control"  readonly/> -->
<!-- 	            </td> -->
