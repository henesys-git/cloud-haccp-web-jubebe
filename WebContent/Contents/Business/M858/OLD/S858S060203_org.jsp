...
String[] strColumnHead 	= { "주문번호","출고번호","입고일자","입고일련번호","입고시간",
	...
	String GV_CALLER="",GV_ORDERNO="",GV_PROD_CD_REV="",GV_PROD_CD="",GV_IPGONO="";
	...
	if(request.getParameter("PartCdRev")== null)
		...
		GV_PROD_CD_REV = request.getParameter("PartCdRev");
	 ...
	 */
	String param = GV_ORDERNO + "|" + GV_IPGONO + "|" + GV_PROD_CD + "|" + GV_PROD_CD_REV + "|";
// 	TableHead.getValueAt(0,0).toString()
    TableModel = new DoyosaeTableModel("M858S060100E184", strColumnHead, param);	//검수정보를 가져와야 함
    ...
    makeTableHTML.htmlTable_ID	= "TableS858S060103";
...
<script>
    ...
    function SaveOderInfo() {        
		...
		var parmHead= "" 
			...
			+ '<%=GV_PROD_CD%>' 					+ "|" 
			+ '<%=GV_PROD_CD_REV%>' 				+ "|"
			+ '<%=TableModel.getValueAt(0,6).toString().trim()%>' + "|"	//4 제품코드
	    	+ '<%=TableModel.getValueAt(0,7).toString().trim()%>' + "|"	//5 제품개정번호
	    	...
	    	+ '<%=TableModel.getValueAt(0,14).toString().trim()%>' + "|"	//1 입출고 수량
 	    	...
 	    	+ '<%=TableModel.getValueAt(0,1).toString().trim()%>'+ "|"	//2 출고번호
 	    	+ '<%=member_key%>' + "|"
			+ "<%=Config.HEADTOKEN %>" 	;
