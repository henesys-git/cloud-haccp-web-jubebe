...
<%
	...
	String[] strColumnHead 	= {"창고","렉","선반","칸","제품코드","개정번호","입고전재고","재고","입고수"};
	int[]   colOff 			= {1,1,1,1,0,0,0,1,0};//전체 넓이 번튼 3개:86%, 문서2개:90%, 문서뷰:94%, 챠트보기,문서등록91%; 고객사
...
// 		GV_PROD_CD_REV = request.getParameter("prod_cd_rev");
    ...
    makeTableHTML.htmlTable_ID	= "TableS858S060120";
...
<script>
    $(document).ready(function () {
		...
		vTableS858S060120 = $('#<%=makeTableHTML.htmlTable_ID%>').DataTable({
			scrollX: true,
		    scrollY: 340,
		    scrollCollapse: true,
		    paging: false,
...
// 		    keys: false,
		...
		$('#<%=makeTableHTML.htmlTable_ID%> tbody').on( 'click', 'tr', function () {
			S858S060120_Row_index = vTableS858S060120
		        .row( this )
		        .index();
			
			
// 			$($("input[id='checkbox1']")[S858S060120_Row_index]).prop("checked", function(){
// 		        return !$(this).prop('checked');
// 		    });

	 		$(<%=makeTableHTML.htmlTable_ID%>_rowID).removeClass("hene-bg-color");
			vTableS858S060120.row(this)
		    .nodes()
		    .to$()
		    .attr("class","hene-bg-color");
	
			$('#txt_store_no').val(vTableS858S060120.cell(S858S060120_Row_index, 0).data());	// machineno
			$('#txt_rakes_no').val(vTableS858S060120.cell(S858S060120_Row_index, 1).data());	// rakes
			$('#txt_plate_no').val(vTableS858S060120.cell(S858S060120_Row_index, 2).data());	// plate
			$('#txt_colm_no').val(vTableS858S060120.cell(S858S060120_Row_index, 3).data());		// colm
