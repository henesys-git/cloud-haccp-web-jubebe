...
//     TableModel = new DoyosaeTableModel("M202S010100E244", strColumnHead, param);
    ...
    makeTableHTML.htmlTable_ID	= "TableS202S010255";
...
<script>
    $(document).ready(function () {
		...
		vTableS202S010255 = $('#<%=makeTableHTML.htmlTable_ID%>').removeAttr('width').DataTable({
			scrollX: true,
		    scrollY: 380,
		    scrollCollapse: true,
		    paging: false,
...
// 	      		$(row).attr('onclick',"S202S010240Event(this)");
		...
		$('#<%=makeTableHTML.htmlTable_ID%> tbody').on( 'click', 'tr', function () {
			S202S010255_Row_index = vTableS202S010255
		        .row( this )
		        .index();
			

	 		$(<%=makeTableHTML.htmlTable_ID%>_rowID).removeClass("hene-bg-color");
			vTableS202S010255.row(this)
		    .nodes()
		    .to$()
		    .attr("class","hene-bg-color");
			/*
			$('#txt_order_no').val(vTableS202S010255.cell(S202S010255_Row_index, 0).data());		//0  주문번호
			$('#txt_balju_no').val(vTableS202S010255.cell(S202S010255_Row_index, 1).data());		//1 발주번호
			$('#txt_seq').val(vTableS202S010255.cell(S202S010255_Row_index, 2).data());	//2 순번
			$('#txt_bom_nm').val(vTableS202S010255.cell(S202S010255_Row_index, 3).data());			//3 자료명
			$('#txt_bom_cd').val(vTableS202S010255.cell(S202S010255_Row_index, 4).data());	//4 자료번호			
			$('#txt_part_cd').val(vTableS202S010255.cell(S202S010255_Row_index, 5).data());	//6 파트코드
			$('#txt_gyugeok').val(vTableS202S010255.cell(S202S010255_Row_index, 6).data());	//7 규격
			$('#txt_part_cnt').val(vTableS202S010255.cell(S202S010255_Row_index, 7).data());		//8 수량
			$('#txt_unit_price').val(vTableS202S010255.cell(S202S010255_Row_index, 8).data());		//9 단가
			$('#txt_part_amt').val(vTableS202S010255.cell(S202S010255_Row_index, 9).data());	//0 금액
			$('#txt_rev').val(vTableS202S010255.cell(S202S010255_Row_index, 10).data());		//1 rev
			$('#txt_part_cd_rev').val(vTableS202S010255.cell(S202S010255_Row_index, 11).data());			//2 part_cd_revㄴ
			*/
			
			$('#txt_balju_no').val(vTableS202S010255.cell(S202S010255_Row_index, 0).data());		//1 발주번호
			$('#txt_seq').val(vTableS202S010255.cell(S202S010255_Row_index, 2).data());	//2 순번
			$('#txt_bom_nm').val(vTableS202S010255.cell(S202S010255_Row_index, 3).data());			//3 자료명
			$('#txt_bom_cd').val(vTableS202S010255.cell(S202S010255_Row_index, 4).data());	//4 자료번호			
			$('#txt_part_cd').val(vTableS202S010255.cell(S202S010255_Row_index, 5).data());	//6 파트코드
