...
<%
	...
	String[] TR_Style		= {"",""};
// 	String[] TR_Style		= {""," onclick='S202S010220Event(this)' "};
...
//     TableModel = new DoyosaeTableModel("M202S010100E224", strColumnHead, param);	
    ...
    makeTableHTML.htmlTable_ID	= "TableS202S010220";
...
<script>
    $(document).ready(function () {
		...
		vTableS202S010220 = $('#<%=makeTableHTML.htmlTable_ID%>').DataTable({
			scrollX: true,
		    scrollY: 380,
		    scrollCollapse: true,
		    paging: false,
...
// 	      		$(row).attr('onclick',"S202S010220Event(this)");
	      		$(row).attr('role',"row");
	  		},
	  		'columnDefs': [{
	       		'targets': [0,1,2,11],
		...
		$('#<%=makeTableHTML.htmlTable_ID%> tbody').on( 'click', 'tr', function () {
			S202S010220_Row_index = vTableS202S010220
		        .row( this )
		        .index();
...
// 		    });
			...
			vTableS202S010220.row(this)
		    .nodes()
		    .to$()
		    .attr("class","hene-bg-color");
			
			$('#txt_order_no').val(vTableS202S010220.cell(S202S010220_Row_index, 0).data());		//0 BOM 주문번호
			$('#txt_bom_cd').val(vTableS202S010220.cell(S202S010220_Row_index, 1).data());		//1 BOM코드
			$('#txt_bom_nm').val(vTableS202S010220.cell(S202S010220_Row_index, 2).data());	//2 BOM명
			$('#txt_seq').val(vTableS202S010220.cell(S202S010220_Row_index, 3).data());			//3 순번
			$('#txt_part_cd').val(vTableS202S010220.cell(S202S010220_Row_index, 5).data());	//7 파트코드
			$('#txt_gyugeok').val(vTableS202S010220.cell(S202S010220_Row_index, 6).data());		//8 규격
			$('#txt_part_cnt').val(vTableS202S010220.cell(S202S010220_Row_index, 7).data());		//9 수량
			$('#txt_unit_price').val(vTableS202S010220.cell(S202S010220_Row_index, 8).data());	//0 단가
			$('#txt_part_amt').val(vTableS202S010220.cell(S202S010220_Row_index, 9).data());		//1 금액
			$('#txt_rev').val(vTableS202S010220.cell(S202S010220_Row_index, 10).data());			//2 구분
			$('#txt_part_cd_rev').val(vTableS202S010220.cell(S202S010220_Row_index, 11).data());			//2 구분
