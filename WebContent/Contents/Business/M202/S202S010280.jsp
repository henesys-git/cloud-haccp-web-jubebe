 ...
 */
	...
	String htmlTable_ID	= "TableS202S010280";
	
	int DataCount = 0;
 	// 표에 들어갈 데이터 -> 자바스크립트 배열로 가공
	DataArray.append("[");
	for(int i = 0; i < RowCount; i++) {
		BigDecimal CNT_POST_AMT = new BigDecimal(TableModel.getValueAt(i, 3).toString().trim());
		BigDecimal CNT_SAFETY_JAEGO = new BigDecimal(TableModel.getValueAt(i, 4).toString().trim());
		BigDecimal CNT_BALJU_COUNT = new BigDecimal("0");
		for(int j=0; j<jjArray.size(); j++) { 
			...
			if(TableModel.getValueAt(i, 0).toString().trim().equals(jjjArray.get("part_cd")))
				CNT_BALJU_COUNT = new BigDecimal(jjjArray.get("balju_count").toString());
		}
		if(CNT_POST_AMT.add(CNT_BALJU_COUNT).compareTo(CNT_SAFETY_JAEGO) == -1 ) { // 현재재고+발주수량<안전재고 : 경고
			DataArray.append("[");
			...
			DataArray.append( "'" + CNT_SAFETY_JAEGO.subtract(CNT_POST_AMT.add(CNT_BALJU_COUNT)) + "'" + "" ); // 예정 부족수량(안전재고-(현재재고+발주수량))
			if(i == RowCount-1) {
				DataArray.append("]");
			} else {
				DataArray.append("],");
...
<script>
	...
	<p style="text-align:center">
		<button id="btn_Save" class="btn btn-info" onclick="Save_S202S010280();">저장</button>
