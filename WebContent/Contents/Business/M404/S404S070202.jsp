...
<%
	...
	String[] strColumnHead 	= {"주문번호","주문상세번호", "고객사코드", "고객사명","주문일자","출하시리얼번호", "납기일자",  "프로젝트명","고객사PO번호",
			"출하코드","제품명", "주문수량","롯트번호", "롯트수량","비고","프로젝트수량"};
	...
	function SaveOderInfo() {        
        ...
        for(var i=0; i<detail_seq;i++){
			...
			<%-- parmHead += '<%=GV_JSPPAGE%>' 				+ "|" 
					...
					dataJson.order_detail 		= trInput.eq(0).val(); 			//주문상세번호
					...
					dataJson.data_delevery 		= $('#dateDelevery').val();
					dataJson.product_serial_no 	= $('#txt_productSerialNo').val()
					dataJson.jumun_count 		= $('#txt_jumunCount').val() 	//order_count
					dataJson.cust_poNo 			= $('#txt_cust_poNo').val()
					dataJson.bigo 				= $('#txt_bigo').val()
					dataJson.chulha_code 		= trInput.eq(3).val() 			//출하코드
					dataJson.prod_rev 			= trInput.eq(4).val() 			//prod_rev
					dataJson.lot_no 			= trInput.eq(5).val() 			//LOT No
					dataJson.lot_count 			= trInput.eq(6).val() 			//LOT수량
					dataJson.projrct_cnt 		= 	$('#txt_projrctCnt').val()
					
					if($('#txt_DelYN').is(":checked"))
						dataJson.yes_or_no = "Y";
            		else
            			dataJson.yes_or_no = "N";
		...
		<td style="width: 800px;">
	        <table class="table table-striped" style="width: 100%; margin: 0 auto; align:left">
		        ...
		        <tr style="vertical-align: middle">
		            ...
		            <td style="width: 140px; font-weight: 900; font-size:14px; text-align:center; vertical-align: middle">출하코드</td>
