...
$(document).ready(function () {
	vTableS353S020120 = $('#<%=makeGridData.htmlTable_ID%>').DataTable({	
		scrollX: true,
		scrollY: 500,
	    scrollCollapse: true,
	    paging: false,
  		...
  		'columnDefs': [
  			{
       		...
       		},
       		{
           		...
           		'createdCell':  function (td, cellData, rowData, rowinx, col) {
           			$(td).text("");
           			$(td).append("<input type='text' style='width:100%;'value='"+cellData+"'/>");
           		}
       		}
		], 
	    language: { 
	...
	TableS353S020120_RowCount = vTableS353S020120.rows().count();
	if(TableS353S020120_RowCount>0){
		var v_production_date = vTableS353S020120.cell(0,16).data();
		if(v_production_date.length>0) {
			$('#txt_production_date').datepicker('update', v_production_date);
		}
	} else {
		alert("생산실적관리에서 배합(레시피)를 등록하세요");
