 ...
 */
	...
	String[] TR_Style		= {""," onclick='S101S030995Event(this)' "};
    ...
    TableModel = new DoyosaeTableModel("M101S030100E995", strColumnHead, jArray);
    ...
    makeTableHTML.htmlTable_ID	= "TableS101S030995";
...
<script type="text/javascript">	
	var TableS101S030995_Row_Index;
	var vTableS101S030995;
	var Rowcount='<%=RowCount%>';
	$(document).ready(function () {
		vTableS101S030995 = $('#<%=makeTableHTML.htmlTable_ID%>').DataTable({
			scrollX: true,
		    scrollY: 380,
		    scrollCollapse: true,
		    paging: false,
...
// 	      		$(row).attr('onclick',"S101S030995Event(this)");
// 	      		$(row).attr('role',"row");
// 	  		},
// 	  		'columnDefs': [{
// 	       		'targets': [5,12,13,14],
...
// 			}]	 
		...
		$('#<%=makeTableHTML.htmlTable_ID%> tbody').on( 'click', 'tr', function () {
			TableS101S030995_Row_Index = vTableS101S030995
		        .row( this )
		        .index();
	 		$(<%=makeTableHTML.htmlTable_ID%>_rowID).removeClass("hene-bg-color");		
	 		
	 		
// 	 		$($("input[id='checkbox1']")[TableS101S030995_Row_Index]).prop("checked", function(){
			vTableS101S030995.cell(TableS101S030995_Row_Index,0).nodes().to$().find("input[id='checkbox1']").prop("checked", function(){
...
// // 		$(obj).attr("class", "bg-success"); 
    ...
    function Save_S101S030995(){
        var table_info = vTableS101S030995.page.info();
...
//     		if($(this).is(":checked")) $("input:checkbox[id='chkBox']").is(":checked") ;
    		...
    		if($($("input:checkbox[id='checkbox1']")[i]).is(":checked") ){
    			...
    			vTableS101S030990.row.add( [
    				$('#txt_orderno').val(),
    				$('#txt_orderdetailseq').val(),
    				vTableS101S030995.cell(i , 18).data(),	//18 검사구분  
    				vTableS101S030995.cell(i , 19).data(),	//18 구분명  
    				'',										//   주문별체크번호, order_check_no
    				'',										//   revision_no    
    				vTableS101S030995.cell(i , 2).data(),	//1 제품코드           
    				vTableS101S030995.cell(i , 3).data(),	//2 제품명        
    				vTableS101S030995.cell(i ,10).data(),	//9 prod_cd_rev   
    				
    				vTableS101S030995.cell(i , 4).data(),	//3 체크코드       
    				vTableS101S030995.cell(i ,11).data(),	//0 checklist_seq    
    				vTableS101S030995.cell(i ,12).data(),	//1 checklist_cd_rev
    				
    				vTableS101S030995.cell(i , 5).data(),	//4 작업표준         
    				vTableS101S030995.cell(i , 7).data(),	//6 표준값   					 
    				vTableS101S030995.cell(i , 6).data(),	//5 체크내용  
	...
	<p style="text-align:center">
		<button id="btn_Save"  class="btn btn-info"  onclick="Save_S101S030995();">적용</button>
