...
//  웹소켓 통신을 위해서 필요한 변수들 ---끝	
	...
	var vTableS101S030970;
	var TableS101S030970_info;
    var TableS101S030970_Row_Count;
    var TableS101S030970_Row_Index;
...
// 			{"순번" ,"공정번호","공정명","부서", "proc_rev","체크리스코드","checklist_rev","작업표준","공구","관리항목","표준값"};
			if(TableS101S030970_Row_Index>-1){
			    vTableS101S030970.cell( TableS101S030970_Row_Index, 0 ).data( $('#txt_checklist_seq').val() );
			    vTableS101S030970.cell( TableS101S030970_Row_Index, 1 ).data( $('#txt_proc_cd').val() );
			    vTableS101S030970.cell( TableS101S030970_Row_Index, 2 ).data( $('#txt_process_name').val() );
			    vTableS101S030970.cell( TableS101S030970_Row_Index, 3 ).data( $('#txt_dept_gubun').val() );
			    vTableS101S030970.cell( TableS101S030970_Row_Index, 4 ).data( $('#txt_proc_rev').val() );
			    vTableS101S030970.cell( TableS101S030970_Row_Index, 5 ).data( $('#txt_checklist_cd').val() );
			    vTableS101S030970.cell( TableS101S030970_Row_Index, 6 ).data( $('#txt_checklist_cd_rev').val() );
			    vTableS101S030970.cell( TableS101S030970_Row_Index, 7 ).data( $('#txt_standard_guide').val() );
			    vTableS101S030970.cell( TableS101S030970_Row_Index, 8 ).data( $('#txt_tools').val() );
			    vTableS101S030970.cell( TableS101S030970_Row_Index, 9 ).data( $('#txt_check_note').val() );
			    vTableS101S030970.cell( TableS101S030970_Row_Index, 10 ).data( $('#txt_standard_value').val() );
			    vTableS101S030970.cell( TableS101S030970_Row_Index, 22 ).data( $('#txt_modify_reason').val() );
			    vTableS101S030970.cell( TableS101S030970_Row_Index, 23 ).data( $('#txt_revision_no').val() );
// 			    vTableS101S030970.row(TableS101S030970_Row_Index).draw(); 
			}
			else{
//	 			{"순번" ,"공정번호","공정명","부서", "proc_rev","체크리스코드","checklist_rev","작업표준","공구","관리항목","표준값"};
				vTableS101S030970.row.add( [
					...
					$('#txt_modify_reason').val(),
					$('#txt_revision_no').val(),//표준값
		    ...
		    revCheck[TableS101S030970_Row_Index]=true;
// 		    alert("rev=" + revCheck[TableS101S030970_Row_Index] + "\n" + TableS101S030970_Row_Index)
		    
	        TableS101S030970_info = vTableS101S030970.page.info();
	        TableS101S030970_Row_Count = TableS101S030970_info.recordsTotal;
	        $('#txt_seq').val( TableS101S030970_Row_Count + 1);
// 	        $('#txt_lastseq').val(TableS101S030970_Row_Count);
    ...
    function S101S030970_query(){

		$.ajax({
            type: "POST",
            url: "<%=Config.this_SERVER_path%>/Contents/Business/M101/S101S030970.jsp", 
            data: "order_no=" + '<%=GV_ORDER_NO%>'  + "&order_detail_seq=" + '<%=GV_ORDER_DETAIL_SEQ%>' + "&lotno=" + vLotNo,
            beforeSend: function () {
                $("#bom_tbody").children().remove();
            },
            success: function (html) {           	 
      			$("#bom_tbody").hide().html(html).fadeIn(100);
      			if(vTableS101S030970 != undefined){
      				TableS101S030970_info = vTableS101S030970.page.info();
          	     	TableS101S030970_Row_Count = TableS101S030970_info.recordsTotal;
       		   		rCount = TableS101S030970_Row_Count + 1;
       		   		
       		   		for(i=0;i<TableS101S030970_Row_Count;i++){
       		   			revCheck[i]=false;
       		   		}
      			}
            },
	...
	function SetRecvData(){
		DataPars(M101S030100E922,GV_RECV_DATA);  		
	...
	function SaveOderInfo() {  
		var WebSockData="";
		vTableS101S030970_info = vTableS101S030970.page.info();
		TableS101S030970_RowCount = vTableS101S030970_info.recordsTotal; 
		
		var parmHead= '<%=Config.HEADTOKEN %>' ;

		var parmBody= "" ;
		
	    for(var i=0; i<TableS101S030970_RowCount; i++){   
    		parmHead +=  '<%=JSPpage%>' 					+ "|" 
			+ '<%=loginID%>' 					+ "|" 
			+ $('#txt_num_prefix').val() 		+ "|" 
  			+ $('#txt_orderno').val()			+ "|"	//주문번호
  			+ $('#txt_lotno').val()	+ "|"	//lotno
  			
            + $('#txt_qar').val() 				+ "|"	//qar
            + $('#txt_spec_approval').val()		+ "|"	//spec_approval
            + vTableS101S030970.cell(i , 14).data()	+ "|"	//spec_approval_rev
            + $('#txt_std_dwg').val() 			+ "|"	//std_dwg
            + vTableS101S030970.cell(i , 16).data() 		+ "|"	//std_dwg_rev
            + vTableS101S030970.cell(i , 17).data() 		+ "|"	//program_rev
            + $('#txt_job_guide').val() 		+ "|"	//job_guide
            + vTableS101S030970.cell(i , 19).data()	+ "|"	//job_guide_rev
            
//             {"순번" ,"공정번호","공정명","부서", "proc_rev","체크리스코드","checklist_rev","작업표준","공구","관리항목","표준값"};
	    	+ vTableS101S030970.cell(i , 1).data() + "|"	//  proc_cd   공정번호
	    	+ vTableS101S030970.cell(i , 4).data() + "|"	//  proc_rev  proc_rev					
	    	+ vTableS101S030970.cell(i , 5).data() + "|"	//  checklist_cd 체크리스코드
	    	+ vTableS101S030970.cell(i , 0).data() + "|"	//  checklist_seq 순번
	    	+ vTableS101S030970.cell(i , 6).data() + "|"	//  checklist_rev checklist_rev
	    	+ vTableS101S030970.cell(i , 7).data() + "|"	//  standard_guide 작업표준
	    	+ vTableS101S030970.cell(i , 10).data() + "|"	//  standard_value  표준값
	    	+ vTableS101S030970.cell(i , 8).data() + "|"	//  tools 공구
	    	+ vTableS101S030970.cell(i , 9).data() + "|"	//  check_note 	관리항목
	    	+ vTableS101S030970.cell(i , 22).data() 	+ "|"	//modify_reason
	    	+ vTableS101S030970.cell(i , 11).data() + "|"	//order_check_no
	    	+ vTableS101S030970.cell(i , 20).data()	+ "|"	//start_date
	    	+ vTableS101S030970.cell(i , 21).data()	+ "|"	//create_date
	    	;
	    	
	    	if(revCheck[i]==true){
// 	    		alert("revisioned : " + (parseInt(vTableS101S030970.cell(i , 23).data()) + 1));
	    		parmHead += (parseInt(vTableS101S030970.cell(i , 23).data()) + 1)	+ "|" ;	//revision_no;
	    	}
	    	else {
	    		parmHead += vTableS101S030970.cell(i , 23).data()	+ "|" ;	//revision_no;
	    	}
	    	
	    	parmHead += '<%=Config.DATATOKEN %>' 	;
// 			console.log(parmHead)
	    }		

		SQL_Param.param = parmHead ;
		SendTojsp(parent.urlencode(parmHead), SQL_Param.PID);
...
//	         	 alert(bomdata);
    ...
    function fn_mius_body(obj){  
    	vTableS101S030970.row($(obj).parents('tr')).remove().draw();
		vTableS101S030970_info = vTableS101S030970.page.info();
		TableS101S030970_RowCount = vTableS101S030970_info.recordsTotal; 
    ...
    function fn_Process_CheckList() { /* 파라메터 알맞게 조정 필요 */
    	...
    	//caller = 0 : 일반화면에서 호출
    	var modalContentUrl  = "<%=Config.this_SERVER_path%>/Contents/Business/M101/S101S030950.jsp"; //?caller="+caller;
    	pop_fn_popUpScr_nd(modalContentUrl, "공정별 체크리스트 조회(S101S030950)", '600px', '80%');
		return false;
     }
        ...
        <table class="table table-striped " style="width: 100%; margin: 0 ; align:left">	        
  			<tr>
	            ...
	            <td colspan="3" style="width: 44.5%;  font-weight: 900; font-size:14px; text-align:lef; margin-left:10px;">
 	            ...
 	            <td style=" font-weight: 900; font-size:14px; text-align:left; vertical-align: middle"></td> <!-- 일련번호 -->
	        ...
	        <tr>
	            ...
	            <td style=" tmargin-left:10px;" colspan="7">
					...
					<input type="hidden"  	id="txt_order_check_no"  />
					<input type="hidden"  	id="txt_start_date"  />
					<input type="hidden"  	id="txt_create_date"  />
	            </td>     
	        </tr>	
       	</table>
...
<!-- 	            <td style="width:  2%; font-size:14px; text-align:center; vertical-align: middle">진행명</td> -->
	        ...
	        <tr>
	            <td style=" font-weight: 900; font-size:14px; text-align:left ;vertical-align: middle">수정 사유</td>
	            <td style=" tmargin-left:10px;" colspan="7">
					<input type="text" class="form-control" id="txt_modify_reason" />
