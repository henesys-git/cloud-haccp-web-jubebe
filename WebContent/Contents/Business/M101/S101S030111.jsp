...
//  웹소켓 통신을 위해서 필요한 변수들 ---끝	
	...
	var vTableS101S030140;
	var TableS101S030140_info;
    var TableS101S030140_Row_Count = 0;
    var TableS101S030140_Row_Index = -1;
...
// 			}
			...
			if(TableS101S030140_Row_Index>-1){ //수정
			    vTableS101S030140.cell( TableS101S030140_Row_Index, 7 ).data( $('#txt_part_cnt').val() );
			    vTableS101S030140.cell( TableS101S030140_Row_Index, 8 ).data( $('#txt_maesu').val() );
			    vTableS101S030140.cell( TableS101S030140_Row_Index, 9 ).data( $('#txt_gubun').val() );
			    vTableS101S030140.cell( TableS101S030140_Row_Index, 10 ).data( $('#txt_qar').val() );
			    vTableS101S030140.cell( TableS101S030140_Row_Index, 11 ).data( $('#txt_inspectequep').val() );
			    vTableS101S030140.cell( TableS101S030140_Row_Index, 12 ).data( $('#txt_package').val() );
			    vTableS101S030140.cell( TableS101S030140_Row_Index, 13 ).data( $('#txt_modify').val() );
			    vTableS101S030140.cell( TableS101S030140_Row_Index, 14 ).data( $('#txt_bom_custcode').val() );
			    vTableS101S030140.cell( TableS101S030140_Row_Index, 15 ).data( $('#txt_bom_CustName').val() );
			    vTableS101S030140.cell( TableS101S030140_Row_Index, 16 ).data( $('#txt_cust_rev').val() );
			    vTableS101S030140.cell( TableS101S030140_Row_Index, 17 ).data( $('#txt_bigo').val() );
// 			    vTableS101S030140.row(TableS101S030140_Row_Index).draw();
			    clear_input();
			} else { //추가
				// part_cd 비교해서 이미 등록된 원부자재는 패스
	   			...
	   			for(i=0; i<TableS101S030140_Row_Count; i++) {
	   				if(v_part_cd==vTableS101S030140.cell(i,3).data()) {
	   					jungbok_chk = true;
	       			}
	   			}
	   			if(!jungbok_chk){
    ...
    function call_S101S030140() {
    	$.ajax({
            ...
            success: function (html) {           	 
      			$("#bom_tbody").hide().html(html).fadeIn(100,function(){ // S101S030140페이지 테이블 다 불러오고 나서
          			TableS101S030140_info = vTableS101S030140.page.info();
       	      		TableS101S030140_Row_Count = TableS101S030140_info.recordsTotal;
...
//         	        right_btn_disable();
    ...
    function fn_mius_body(obj){  
		vTableS101S030140.row($(obj).parents('tr')).remove().draw();
	...
	function SaveOderInfo() {      
        ...
        if(TableS101S030140_Row_Count==0) {
		...
		try {
		    ...
		    for(var i=0; i<TableS101S030140_Row_Count; i++){    		
	    		var trInput = $($("#TableS101S030140 tr")[i]).children();
		        ...
		        dataJson.sys_bom_id 	= vTableS101S030140.cell( i, 0 ).data(); //14 sys_bom_id
		        dataJson.part_cd		= vTableS101S030140.cell( i, 3 ).data(); //17 part_cd
		        dataJson.part_nm			= vTableS101S030140.cell( i, 5 ).data(); //18 part_nm
		        dataJson.part_cd_rev	= vTableS101S030140.cell( i, 6 ).data(); //19 part_cd_rev
		        dataJson.part_cnt		= vTableS101S030140.cell( i, 7 ).data(); //20 part_cnt
// 		        dataJson.mesu			= vTableS101S030140.cell( i, 8 ).data(); //21 mesu
		        dataJson.mesu			= '0'; //21 mesu
		        dataJson.gubun			= vTableS101S030140.cell( i, 9 ).data(); //22 gubun
		        dataJson.qar			= vTableS101S030140.cell( i, 10 ).data(); //23 qar
		        dataJson.inspect_selbi	= vTableS101S030140.cell( i, 11 ).data(); //24 inspect_selbi
		        dataJson.packing_jaryo	= vTableS101S030140.cell( i, 12 ).data(); //25 packing_jaryo
		        dataJson.modify_note	= vTableS101S030140.cell( i, 13 ).data(); //26 modify_note
		        dataJson.cust_code		= vTableS101S030140.cell( i, 14 ).data(); //27 cust_code
// 		        dataJson.cust_rev		= vTableS101S030140.cell( i, 16 ).data(); //28 cust_rev
		        dataJson.cust_rev		= '0'; //28 cust_rev
		        dataJson.bigo 			= vTableS101S030140.cell( i, 17 ).data(); //29 bigo
...
// 	        	 parent.DetailInfo_List.click();
    ...
    function select_Product_Bom_List(){
    	var modalContentUrl = "<%=Config.this_SERVER_path%>/Contents/Business/M101/S101S030145.jsp"
    						+ "?bom_cd=" + "<%=GV_PROD_CD%>"
    						+ "&bom_cd_rev=" + "<%=GV_PROD_CD_REV%>" ;
    	
    	pop_fn_popUpScr_nd(modalContentUrl, "배합(BOM)목록조회(S101S030145)", '600px', '80%');
		return false;
	}
    
    function right_btn_disable(){
        for(var i=0; i<TableS101S030140_Row_Count; i++){
			var trInput = $($("#TableS101S030140 tr")[i]).find(":button")
        ...
        <table class="table table-striped table-bordered" style="width: 100%; margin: 0 ; align:left">	       
	        <tbody id="bom_main_tbody"> 
  				<tr>  				
		            ...
		            <td style="width: 15%;  font-weight: 900; font-size:14px; text-align:right;vertical-align: middle">배합수량</td>
