...
// 	proc_cd, 공정명,	A.checklist_seq,		B.checklist_cd,	B.check_note,	B.standard_guide,	B.standard_value,	C.item_type,	C.item_desc,	C.item_bigo,	A.checklist_cd_rev,A.revision_no
    ...
    TableModel = new DoyosaeTableModel("M101S030100E954", strColumnHead, param);
    ...
    makeTableHTML.htmlTable_ID	= "TableS101S030950";
...
<script type="text/javascript">	
	var TableS101S030950_Row_Index;
	var vTableS101S030950;
	var Rowcount='<%=RowCount%>';
	$(document).ready(function () {
		vTableS101S030950 = $('#<%=makeTableHTML.htmlTable_ID%>').DataTable({
			scrollX: true,
		    scrollY: 380,
		    scrollCollapse: true,
		    paging: false,
		...
		$('#<%=makeTableHTML.htmlTable_ID%> tbody').on( 'click', 'tr', function () {
			TableS101S030950_Row_Index = vTableS101S030950.row( this ).index();
	 		$(<%=makeTableHTML.htmlTable_ID%>_rowID).removeClass("hene-bg-color");		
	 		
	 		vTableS101S030950.cell(TableS101S030950_Row_Index,0).nodes().to$().find("input[id='checkbox1']").prop("checked", function(){
	 		...
	 		vTableS101S030950.row( this ).nodes().to$().attr("class", "hene-bg-color");
		} );
		...
		$('#<%=makeTableHTML.htmlTable_ID%> tbody').on( 'blur', 'tr', function () {
			vTableS101S030950.row( this ).nodes().to$().attr("class", "hene-bg-color_w");
		} );
...
//         	$('#txt_lastseq').val(Rowcount);
    ...
    function Save_S101S030950(){
        var table_info = vTableS101S030950.page.info();
        var table_rows = table_info.recordsTotal;
    	for(var i=0;i<table_rows;i++){    		
//     		if($(this).is(":checked")) $("input:checkbox[id='chkBox']").is(":checked") ;
//S101S030970 {"순번" ,"공정번호","공정명","부서", "proc_rev","체크리스코드","checklist_rev","작업표준","공구","관리항목","표준값"};				
//S101S030950 {"공정번호","공정명","순번","관련부서", "체크코드","관리항목","작업표준" ,"표준값", "항목유형","항목명" ,"비고","checklist_cd_rev","A.proc_cd_rev","revision_no"};
    		if(vTableS101S030950.cell(i,0).nodes().to$().find("input[id='checkbox1']").prop("checked") ){
//     			console.log("input[id='checkbox1']=====" + i);
    			...
    			vTableS101S030970.row.add( [
    				vTableS101S030950.cell(i , 3).data(),	//0 순번
    				vTableS101S030950.cell(i , 1).data(),	//1 공정번호
    				vTableS101S030950.cell(i , 2).data(),	//2 공정명
    				vTableS101S030950.cell(i , 4).data(),	//3 부서
    				vTableS101S030950.cell(i ,13).data(),	//4 proc_rev					
    				vTableS101S030950.cell(i , 5).data(),	//5 체크리스코드
    				vTableS101S030950.cell(i ,12).data(),	//6 checklist_rev
    				vTableS101S030950.cell(i , 7).data(),	//7 작업표준
    				vTableS101S030950.cell(i ,10).data(),	//8 공구
    				vTableS101S030950.cell(i , 6).data(),	//9 관리항목
    				vTableS101S030950.cell(i , 8).data(),	//0 표준값
    				'',
    				'',
    				'',
    				'',
	...
	<p style="text-align:center">
		<button id="btn_Save"  class="btn btn-info"  onclick="Save_S101S030950();">적용</button>
