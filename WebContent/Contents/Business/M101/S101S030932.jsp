...
<%
/*
	S101S030932.jsp : 주문별 제품검사체크리스트 등록 화면
*/
...
//  웹소켓 통신을 위해서 필요한 변수들 ---끝	
	...
	var vTableS101S030990;
	var TableS101S030990_info;
    var TableS101S030990_Row_Count;
    var TableS101S030990_Row_Index;
    var vinspect_gubun=""; 
...
// 	    	Get_S101S030990();
		...
		$('#btn_plus').on( 'click', function () {
		    ...
		    vTableS101S030990.cell( TableS101S030990_Row_Index, 9 ).data( $('#txt_part_cnt').val() );
		    vTableS101S030990.cell( TableS101S030990_Row_Index, 10 ).data( $('#txt_maesu').val() );
		    vTableS101S030990.cell( TableS101S030990_Row_Index, 11 ).data( $('#txt_gubun').val() );
		    vTableS101S030990.cell( TableS101S030990_Row_Index, 12 ).data( $('#txt_qar').val() );
		    vTableS101S030990.cell( TableS101S030990_Row_Index, 13 ).data( $('#txt_inspectequep').val() );
		    vTableS101S030990.cell( TableS101S030990_Row_Index, 14 ).data( $('#txt_package').val() );
		    vTableS101S030990.cell( TableS101S030990_Row_Index, 15 ).data( $('#txt_modify').val() );
		    vTableS101S030990.cell( TableS101S030990_Row_Index, 16 ).data( $('#txt_bom_custcode').val() );
		    vTableS101S030990.cell( TableS101S030990_Row_Index, 17 ).data( $('#txt_bom_CustName').val() );
		    vTableS101S030990.cell( TableS101S030990_Row_Index, 18 ).data( $('#txt_cust_rev').val() );
		    vTableS101S030990.cell( TableS101S030990_Row_Index, 19 ).data( $('#txt_bigo').val() );
// 			    vTableS101S030990.row(TableS101S030990_Row_Index).draw(); 

		    clear_input();
		    
	        TableS101S030990_info = vTableS101S030990.page.info();
	        TableS101S030990_Row_Count = TableS101S030990_info.recordsTotal;
	        $('#txt_seq').val( TableS101S030990_Row_Count + 1);
// 	        $('#').val(TableS101S030990_Row_Count);

	    } );		

    });
    
	function Get_S101S030990(){

		$.ajax({
            type: "POST",
            url: "<%=Config.this_SERVER_path%>/Contents/Business/M101/S101S030990.jsp", 
...
//     	        TableS101S030990_info = vTableS101S030990.page.info();
//    	      		TableS101S030990_Row_Count = TableS101S030990_info.recordsTotal;
   		   		rCount = TableS101S030990_Row_Count + 1;
            },
	...
	function SetRecvData(){
		DataPars(M101S030100E932,GV_RECV_DATA);  		
	...
	function SaveOderInfo() {        
		var WebSockData="";
		vTableS101S030990_info = vTableS101S030990.page.info();
		TableS101S030990_RowCount = vTableS101S030990_info.recordsTotal;
		
		if(TableS101S030990_RowCount == 0 ) { 
	    ...
	    for(var i=0; i<TableS101S030990_RowCount; i++){   
    		parmHead +=  '<%=JSPpage%>' 		+ "|" 	// 0
			+ '<%=loginID%>' 					+ "|" 	// 1
			+ '<%=GV_GET_NUM_PREFIX%>' 			+ "|" 	// 2
	    	...
	    	+ vTableS101S030990.cell(i , 6).data() + "|"	//5 prod_cd   제품코드
	    	+ vTableS101S030990.cell(i , 8).data() + "|"	//6 prod_rev  prod_cd_rev
	    	+ vTableS101S030990.cell(i , 9).data() + "|"	//7 checklist_cd 체크코드
	    	+ vTableS101S030990.cell(i ,10).data() + "|"	//8 checklist_seq checklist_seq
	    	+ vTableS101S030990.cell(i ,11).data() + "|"	//9 checklist_rev checklist_rev
	    	+ vTableS101S030990.cell(i ,12).data() + "|"	//10 standard_guide 작업표준
	    	+ vTableS101S030990.cell(i ,13).data() + "|"	//11 standard_value 	표준값
	    	+ vTableS101S030990.cell(i ,14).data() + "|"	//12 check_note 	체크내용
			+ vTableS101S030990.cell(i ,2).data()  + "|" 	//13 vinspect_gubun 	검사구분
			+ vTableS101S030990.cell(i ,4).data()  + "|" 	//14 order_check_no 	order_check_no
			+ vTableS101S030990.cell(i ,15).data()  + "|" 	//15 order_check_no 	order_check_no
			+ '<%=Config.DATATOKEN %>' 	;
	    }		
...
// 		}
		...
		vTableS101S030990.row($(obj).parents('tr')).remove().draw();
    ...
    function fn_Product_inspect_CheckList() { /* 파라메터 알맞게 조정 필요 */
    	...
    	var modalContentUrl  = "<%=Config.this_SERVER_path%>/Contents/Business/M101/S101S030995.jsp?prod_cd=<%=GV_PROD_CD%>" + "&inspect_gubun=" +vinspect_gubun;
    	pop_fn_popUpScr_nd(modalContentUrl, "제품검사 체크리스트 조회(S101S030995)", '600px', '80%');
		return false;
     }
