...
//  웹소켓 통신을 위해서 필요한 변수들 ---끝	
    ...
    $(document).ready(function () {
    	...
    	$('#txt_package_no').val('<%=TableModel.getValueAt(0,2).toString().trim()%>');
   		$('#txt_start_dt').data('daterangepicker').setStartDate('<%=TableModel.getValueAt(0,3).toString().trim()%>');
   		$('#txt_finish_dt').data('daterangepicker').setStartDate('<%=TableModel.getValueAt(0,4).toString().trim()%>');
...
<%-- 		if("<%=GV_INSPECT_YN%>"=="N" && "<%=GV_INSPECT_REQUEST_YN%>"=="N"){ --%>
// 			alert("자주검사나 품질검사요청 중 하나 이상 완료되야 등록 가능합니다.");
// 			return;
// 		}
	   	...
	   	<tr>   
	        ...
	        <tr style="height: 60px">
	            <td align="center" colspan="2">
	                <p>
	                	<button id="btn_Save"  class="btn btn-info" style="width: 100px" onclick="SaveOderInfo();">수정</button>
