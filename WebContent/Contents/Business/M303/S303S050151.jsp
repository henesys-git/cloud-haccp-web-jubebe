...
<%
	...
	String GV_PROC_PLAN_NO="", GV_PROD_CD="", GV_PROD_CD_REV="", GV_PROD_NM="",
			GV_PART_CD="", GV_PART_CD_REV="", GV_IO_AMT="", GV_IO_AMT_GRAM="", 
			GV_WAREHOUSING_DATETIME="", GV_PRODUCTION_END_DT="";
	...
	if(request.getParameter("io_amt_gram")== null)
		GV_IO_AMT_GRAM="";
	else
		GV_IO_AMT_GRAM = request.getParameter("io_amt_gram");
...
// 		BigDecimal GV_PART_CNT = new BigDecimal(TableModel.getValueAt(i,13).toString().trim());
		...
		jArrayPart.put( "production_end_dt", GV_PRODUCTION_END_DT);
		DoyosaeTableModel TableModelPart = new DoyosaeTableModel("M303S050100E156", jArrayPart); //원재료별 재고현황
		...
		for(int j=0; j<RowCountPart; j++){
			...
			try{ // 재고불출처리
				...
				dbServletLink.connectURL("M303S050100E151"); // 쿼리메소드(PID) 연결
...
// 				} else if(GV_PART_CNT_temp.compareTo(BigDecimal.ZERO)<=0 || j==RowCountPart-1) {//남은 불출수량이 0보다 크지 않으면(또는 마지막 재고면) 불출수량=GV_PART_CNT
				...
				jObject.put( "part_cnt", GV_IO_AMT_GRAM);
				jObject.put( "proc_plan_no", GV_PROC_PLAN_NO);
