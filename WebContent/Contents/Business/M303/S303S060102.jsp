...
<%
	...
	String[] strColumnHead 	= {"주문번호","주문상세번호", "고객사코드", "고객사명","주문일자","제품시리얼번호", "납기일자",  "프로젝트명","고객사PO번호",
			"제품코드","제품명", "주문수량","롯트번호", "롯트수량","비고","프로젝트수량"};
...
//         $($("select[id='select_status']")[1]).prop("selected", true);
	...
	function SaveOderInfo() {        
        ...
        for(var i=0; i<detail_seq;i++){
	    	...
	    	dataJson.projrctname = $('#txt_projrctName').val()
	    	dataJson.custcode = $('#txt_custcode').val()
	    	dataJson.custrev = $('#txt_cust_rev').val()
	    	dataJson.date_order = $('#dateOrder').val() 
	    	dataJson.datedelivery = $('#dateDelevery').val()
	    	dataJson.productSerialNo = $('#txt_productSerialNo').val()
	    	dataJson.jumunCount = $('#txt_jumunCount').val()
	    	dataJson.cust_poNo = $('#txt_cust_poNo').val()
	    	dataJson.bigo = $('#txt_cust_poNo').val()
	    	dataJson.cust_cd = $('#txt_bigo').val()
	    	dataJson.prod_rev = trInput.eq(3).val()
	    	dataJson.lotno = trInput.eq(4).val()
	    	dataJson.lotno = trInput.eq(5).val()
	    	dataJson.projrctcnt = trInput.eq(6).val()
	    	dataJson.delyn = $('#txt_projrctCnt').val()
			if($('#txt_DelYN').is(":checked")) dataJson.headtoken ='Y';
            else dataJson.headtoken ='N';
   	...
   	<tr>   
		<td style="width: 50%;">
   			<table class="table table-bordered" style="width: 100%; margin: 0 auto; align:left">
		        ...
		        <tr>
		            <td colspan="4"><textarea class="form-control" id="txt_bigo"  style="cols:10;rows:4" ></textarea></td>
		...
		<td style="width: 800px;">
	        <table class="table table-striped" style="width: 100%; margin: 0 auto; align:left">
		        ...
		        <tr style="vertical-align: middle">
		            ...
		            <td style="width: 140px; font-weight: 900; font-size:14px; text-align:center; vertical-align: middle">제품코드</td>
