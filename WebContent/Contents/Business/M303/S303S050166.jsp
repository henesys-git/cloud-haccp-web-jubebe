 ...
 */
    ...
    TableModel = new DoyosaeTableModel("M303S050100E166", jArray);
    /* =========================복사하여 수정 할 부분====끝=====================================  */  
	...
	String RightButton[][]	= {{"off", "fn_Chart_View", rightbtnChartShow},{"off", "fn_Doc_Reg()", rightbtnDocSave},{"off", "file_real_name", rightbtnDocShow}};
 	...
 	/* =========================복사하여 수정 할 부분=================================================  */ 
    makeGridData.htmlTable_ID	= "TableS303S050166";
    /* =========================복사하여 수정 할 부분====끝=====================================  */  
...
<script type="text/javascript">
	...
	var vTableS303S050166;
	var S303S050166_Row;
	var table_info;
	var table_rows = 0;
	$(document).ready(function () {
		vTableS303S050166 = $('#<%=makeGridData.htmlTable_ID%>').DataTable({
			scrollX: true,
		    scrollY: 380,
		    scrollCollapse: true,
		    paging: false,
...
<%-- 	      		$(row).attr('onclick',"<%=makeGridData.htmlTable_ID%>Event(this)"); --%>
	  		...
	  		'columnDefs': [{
	       		'targets': [3,5,7,9,10,11,12,13,14,15,16,17,19,20,21,22],
		...
		$('#<%=makeGridData.htmlTable_ID%> tbody').on( 'click', 'tr', function () {
			S303S050166_Row = vTableS303S050166.row( this ).index();
	 		$(<%=makeGridData.htmlTable_ID%>_rowID).removeClass("hene-bg-color");		
	 		
	 		vTableS303S050166.cell(S303S050166_Row,0).nodes().to$().find("input[id='checkbox1']").prop("checked", function(){
				if($(this).prop('checked'))
					$(this).prop("checked", false);
				else
					$(this).prop("checked", true);
	 		...
	 		vTableS303S050166.row( this ).nodes().to$().attr("class", "hene-bg-color");
		} );
		
		$("#<%=makeGridData.htmlTable_ID%>_head input[id='checkboxAll']").on("click", function(){
			table_info = vTableS303S050166.page.info();
			table_rows = table_info.recordsTotal;
			
			for(var i=0;i<table_rows;i++){  
				vTableS303S050166.cell(i,0).nodes().to$().find("input[id='checkbox1']").prop("checked",this.checked);
			}
		});
	});
...
// 		$('#txt_bigo').val(td.eq(17).text().trim());
    ...
    function Save_S303S050166(){
        var table_info = vTableS303S050166.page.info();
        var table_rows = table_info.recordsTotal;
    	for(var i=0;i<table_rows;i++){
    		var trInput = vTableS303S050166.cell(i, 0).nodes().to$().find(":input");
    		if($(trInput.eq(0)).is(":checked") ){
    			// part_cd 비교해서 이미 등록된 원부자재는 패스
    			var v_part_cd = vTableS303S050166.cell(i,4).data();
    			var jungbok_chk = false;
    			for(j=0; j<TableS303S050165_Row_Count; j++) {
    				if(v_part_cd==vTableS303S050165.cell(j,3).data()) {
    					jungbok_chk = true;
        			}
    			}
    			// 중복아니면 vTableS303S050165 테이블에 데이터행추가
    			if(!jungbok_chk){
    				vTableS303S050165.row.add( [
    					'', //0 warehousing_datetime
    					'', //1 io_gubun
    					'',	//2 유통기한
    					vTableS303S050166.cell(i,4).data(), //3 part_cd
    					vTableS303S050166.cell(i,7).data(), //4 part_cd_rev
    					vTableS303S050166.cell(i,6).data(), //5 원재료명
    					vTableS303S050166.cell(i,20).data(), //6 규격
    					vTableS303S050166.cell(i,21).data(), //7 규격(g단위)
    					'', //8 불출 전 재고
    					'', //9 불출 후 재고
    					parseFloat(vTableS303S050166.cell(i,8).data())*parseFloat(parent.$("#txt_mix_recipe_cnt").val()), //10 원재료 불출 수량
    					'', //11 proc_plan_no          	
    					'', //12 prod_cd
    					'', //13 prod_cd_rev
    					'', //14 prod_nm
    					'', //15 비고 
    					'Y', //16 insert_yn(최초등록여부:Y면등록,N이면수정) 
    	...
    	vTableS303S050165.draw(true);
		vTableS303S050165_info = vTableS303S050165.page.info();
		TableS303S050165_Row_Count = vTableS303S050165_info.recordsTotal;
...
<table class='table table-bordered nowrap table-hover' id="<%=makeGridData.htmlTable_ID%>" style="width: 100%">
		...
		<tr>
		     ...
		     <th style='width:0px; display: none;'>g단위 규격</th>
<!-- 		     버튼 자리 makeGridData의 데이터는 항상 버튼위 위치에 데이터를 space혹은 Button 문법을 구현해 준다 -->
	...
	<p style="text-align:center">
		<button id="btn_Save"  class="btn btn-info"  onclick="Save_S303S050166();">적용</button>
