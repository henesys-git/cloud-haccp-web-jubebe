...
<%
	...
	String RightButton[][]	= {{"on", "fn_inspect_result_delete(this)", "삭제"},{"off", "fn_Doc_Reg()", rightbtnDocSave},{"off", "file_real_name", rightbtnDocShow}};
 	...
 	/* =========================복사하여 수정 할 부분=================================================  */ 
    makeGridData.htmlTable_ID	= "TableS303S050170";
    /* =========================복사하여 수정 할 부분====끝=====================================  */  
...
<script type="text/javascript">
	...
	$(document).ready(function () {
		vTableS303S050170 = $('#<%=makeGridData.htmlTable_ID%>').DataTable({
			scrollX: true,
		    scrollY: 380,
		    scrollCollapse: true,
		    paging: false,
// 		    lengthMenu: [[8,9,-1], [8,9,"All"]],
		    ...
		    'createdRow': function(row) {	
	  		...
	  		'columnDefs': [
	  			{
		       		'targets': [1,2,4,6,8,9,10,11,12,13,14,15,16,18,19,20,21,22,23,24],
...
// 		          		$(td).html('<button style="width: auto; float: left; " type="button" id="right_btn" onclick="fn_inspect_result_delete(this)" class="btn-outline-success">삭제</button>'); 
		...
		$('#<%=makeGridData.htmlTable_ID%> tbody').on( 'click', 'tr', function () {
			TableS303S050170_Row_Index = vTableS303S050170
		        .row( this )
		        .index();
			$('#btn_plus').html("수정");
		} );
		if(Rowcount > 0){
			...
			$('#txt_bom_cd').val(		vTableS303S050170.cell( 0, 1 ).data());	//4 bom_cd 
			$('#txt_bom_cd_rev').val(	vTableS303S050170.cell( 0, 2 ).data());	//5 bom_cd_rev
// 			$('#txt_bom_name').val(	vTableS303S050170.cell( 0, 4 ).data());	//6 bom_name
			$('#txt_type_code').val(	vTableS303S050170.cell( 0, 19 ).data());	//9 type_no 형식번호
			$('#txt_A_productname').val(vTableS303S050170.cell( 0, 20 ).data());	//10 geukyongpoommok 적용품목
			$('#txt_deptcode').val(	vTableS303S050170.cell( 0, 22 ).data());	//11 dept_code 부서코드
// 			$('#txt_deptcode').val(	vTableS303S050170.cell( 0, 22 ).data()).prop("selected",true);  //11 dept_code 부서코드
			$('#approval_date').val(	vTableS303S050170.cell( 0, 21 ).data());	//12 approval_date 승인일자
			$('#approval').val(		vTableS303S050170.cell( 0, 23 ).data());	//13 approval 승인
		}
	});
