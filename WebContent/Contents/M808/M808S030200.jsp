...
%>
 ...
 <script type="text/javascript">
    ...
    $(document).ready(function () {
		...
		$("#InfoContentTitle").html("한계이탈데이터인증원전송목록");
	...
	function fn_DetailInfo_List() {
    	$.ajax(
    	    {
    	        type: "POST",
    	        url: "<%=Config.this_SERVER_path%>/Contents/Business/M808/S808S030210.jsp",
...
//     	            $("#ProcessInfo_bom").children().remove();
    ...
    function pop_fn_SeolbiInfo_Insert(obj) {
    	if(vSeolbiCd.length < 1){
			$('#alertNote').html(obj.innerText + " <BR> <%=JSPpage%>!!! <BR><BR> 한계이탈데이터인증원전송 정보를 선택하세요  !!!");
			$('#modalalert').show();
			return false;
		}
    	var modalContentUrl;
    	modalContentUrl = "<%=Config.this_SERVER_path%>/Contents/Business/M808/S808S030201.jsp?SeolbiCd=" + vSeolbiCd + "&RevisionNo=" + vRevisionNo;
    	
		modalFramePopup.setTitle(obj.innerText);
		pop_fn_popUpScr(modalContentUrl, obj.innerText+"(S808S030201)", "550px", "800px");
     }
    ...
    function pop_fn_SeolbiInfo_Update(obj) {
    	...
    	modalContentUrl = "<%=Config.this_SERVER_path%>/Contents/Business/M808/S808S030202.jsp?SeolbiCd=" + vSeolbiCd + "&SeqNo=" + vSeqNo + "&RevisionNo=" + vRevisionNo;
    	
		modalFramePopup.setTitle(obj.innerText);
		pop_fn_popUpScr(modalContentUrl, obj.innerText+"(S808S030202)", "550px", "800px");
// 		var rtnVal = fn_CommonPopupModal(modalContentUrl, null, 450, 600); modal-md 
    ...
    function pop_fn_SeolbiInfo_Delete(obj) {
    	...
    	modalContentUrl = "<%=Config.this_SERVER_path%>/Contents/Business/M808/S808S030203.jsp?SeolbiCd=" + vSeolbiCd + "&SeqNo=" + vSeqNo + "&RevisionNo=" + vRevisionNo;
    	
		modalFramePopup.setTitle(obj.innerText);
		pop_fn_popUpScr(modalContentUrl, obj.innerText+"(S808S030203)", "550px", "800px");
		return false;
    }
    ...
    <div class="panel panel-default" style="margin-left:5px; margin-top:5px; margin-right:5px; margin-bottom:5px; width:100%">
        ...
        <div class="panel-heading" style="font-weight: bold" id="MenuTitle">여기에한계이탈데이터인증원전송타이틀</div>
        <div class="panel-body">
            <div>
            
	            <button data-author="mainBtn" type="button"  id="mainButton"  class="btn btn-primary active"
	            			style="width: auto; float: left; ">HACCP관리-한계이탈데이터인증원전송</button>
	            			
	            <button data-author="update" type="button" onclick="pop_fn_SeolbiInfo_Update(this)" id="update" class="btn btn-outline-success" 
	            			style="width: auto; float: left; margin-left:3px;">한계이탈데이터인증원 전송</button>
            </div>
            <p style="width: auto; clear:both;">
            </p>
            <div class="panel panel-default">
                <!-- Default panel contents -->
                <div class="panel-heading" style="font-size:16px; font-weight: bold" id="InfoContentTitle">한계이탈데이터인증원전송목록</div>
                <div class="panel-body">
                ...
                <div class="panel-heading" style="font-size:16px; font-weight: bold " id="JumunInfoContentTitle2">한계이탈데이터인증원전송상세</div>
                <div class="panel-body">
