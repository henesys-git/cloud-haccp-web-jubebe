...
%>
 ...
 <script type="text/javascript">
    ...
    $(document).ready(function () {
		...
		$("#InfoContentTitle").html("외부인 출입기록");
...
// 		//$( function() {$( "#tabs" ).tabs();});
    ...
    function fn_MainInfo_List() {
        ...
        {
            type: "POST",
            url: "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S050400.jsp",
...
//     	vExtnlInDate	= $(obj).parent().parent().find("td").eq(0).text().trim();
//     	var today = new Date(vExtnlInDate); // 방문일자
...
//     	var vExtnlInDateStart = duration_start.getFullYear() 
...
// 		var vExtnlInDateEnd = duration_end.getFullYear() 
...
//     						 + "?extnl_in_date_start=" + vExtnlInDateStart
//     						 + "&extnl_in_date_end=" + vExtnlInDateEnd ;
    						 + "?page_start=" + 1 ; // 1페이지	
    	
    	pop_fn_popUpScr(modalContentUrl, "<%=sMenuTitle%>"+"(S838S050400_canvas)", '800px', '1260px');
...
<%-- 		fn_HACCP_View("<%=sMenuTitle%>","<%=sProgramId%>"); --%>
	...
	// 외부인출입기록 등록
    function pop_fn_Extnl_Check_Insert(obj) {
		var modalContentUrl = "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S050401.jsp";
<%--     						+ "?jspPage=" + "<%=JSPpage%>" --%>
    			
		pop_fn_popUpScr(modalContentUrl, obj.innerText+"(S838S050401)", "90%", "40%");
    }    
    
    // 외부인출입기록 수정
    function pop_fn_Extnl_Check_Update(obj) {    	
		if(vExtnlInDate.length < 1){
			$('#alertNote').html(obj.innerText + " <BR> <%=JSPpage%>!!! <BR><BR> 점검상세정보 탭에서 데이터를 선택하세요  !!!");
			$('#modalalert').show();
			return false;
		}
    	var modalContentUrl = "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S050402.jsp"
    						+ "?extnl_in_date=" + vExtnlInDate ;

    	pop_fn_popUpScr(modalContentUrl, obj.innerText+"(S838S050402)", "90%", "40%");
		return false;
	}
    
 	// 외부인출입기록 수정
    function pop_fn_Extnl_Check_Delete(obj) {    	
		if(vExtnlInDate.length < 1){
			$('#alertNote').html(obj.innerText + " <BR> <%=JSPpage%>!!! <BR><BR> 점검상세정보 탭에서 데이터를 선택하세요  !!!");
			$('#modalalert').show();
			return false;
		}
    	var modalContentUrl = "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S050403.jsp" 
				    		+ "?extnl_in_date=" + vExtnlInDate ;

    	pop_fn_popUpScr(modalContentUrl, obj.innerText+"(S838S050403)", "90%", "40%");
		return false;
	}
        ...
        <div class="panel-body">
            <div>
	            ...
	            <button data-author="select" type="button" onclick="fn_HACCP_View('<%=sMenuTitle%>','<%=sProgramId%>')" id="select" class="btn btn-outline-success" 
	            			style="width: auto;float: left; margin-left:30px;" >외부인 출입기록부 (양식)</button>
	            <button data-author="insert" type="button" onclick="pop_fn_Extnl_Check_Insert(this)" id="insert" class="btn btn-default" 
	            			style="width: auto;float: left; margin-left:30px;">외부인출입기록 등록</button>      			
	            <button data-author="update" type="button" onclick="pop_fn_Extnl_Check_Update(this)" id="update" class="btn btn-outline-success" 
	            			style="width: auto; float: left; margin-left:3px;">외부인출입기록 수정</button>
	            <button data-author="update" type="button" onclick="pop_fn_Extnl_Check_Delete(this)" id="delete" class="btn btn-danger" 
	            			style="width: auto; float: left; margin-left:3px;">외부인출입기록 삭제</button>
