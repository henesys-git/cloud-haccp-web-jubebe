...
%>
 ...
 <script type="text/javascript">
	var checklist_id = "checklist31";
	var checklist_rev_no = "";
	var regist_date = "";
	var check_date = "";

	var occur_time = "", occur_place = "", unsuit_detail = "", improve_action = "";
    ...
    $(document).ready(function () {
    	...
    	var date = new SetRangeDate("dateParent", "dateRange", 60);
		...
		$("#InfoContentTitle").html("제조시설 및 설비점검표");
    ...
    function fn_MainInfo_List(startDate, endDate) {
        $.ajax({
            type: "POST",
            url: "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S020900.jsp",
            data: "From=" + startDate + 
            	  "&To=" + endDate,
    ...
    function fn_DetailInfo_List() {     
    	$.ajax({
   	    	type: "POST",
   	        url: "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S020910.jsp",
    ...
    function displayChecklist(element) {    	
    	...
    	if(regist_date.length < 1){
    		heneSwal.warning('조회할 점검표를 선택하세요');
    		return false;
    	}
    ...
    function haccpInsert(obj) {
    	var url = "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S020901.jsp"
    				+ '?checklist_id=' + checklist_id
    				+ '&checklist_rev_no=' + checklist_rev_no;
    ...
    function haccpUpdate(obj) {
    	...
    	if(regist_date.length < 1){
    		heneSwal.warning('수정할 기록을 선택하세요');
    		return false;
    	}
    	
    	var url = "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S020902.jsp"
			+ '?regist_date=' + regist_date 
    ...
    function haccpDelete(obj) {
    	...
    	function SaveOderInfo() {
   			...
   			if(chekrtn) {
   				SendTojsp(JSONparam, "M838S020900E103");
   			} else {
   				return false;
   			}
    	}
...
///////////////////////////////// 상세 점검 기록
    ...
    function haccpPersonInsert(obj) {
    	...
    	if(regist_date.length < 1){
    		heneSwal.warning('점검 기록을 등록할 점검표를 선택하세요');
    		return false;
    	}
    	
    	var url = "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S020911.jsp"
    				+ '?checklist_id=' + checklist_id 
    				+ '&checklist_rev_no=' + checklist_rev_no
    				+ '&regist_date=' + regist_date;
		var footer = '<button id="btn_Save" class="btn btn-info" onclick="SaveOderInfo();">저장</button>';
    ...
    function haccpPersonUpdate(obj) {
    	...
    	if(check_date.length < 1){
    		heneSwal.warning('수정할 점검 기록을 선택하세요');
    		return false;
    	}
    	
    	var url = "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S020912.jsp"
  	...
  	function haccpPersonDelete(obj) {
    	...
    	if(check_date.length < 1){
    		heneSwal.warning('삭제할 점검 기록을 선택하세요');
    		return false;
    	} else{
    		SaveOderInfo();
    	}
    	...
    	function SaveOderInfo() {
    			...
    			if(chekrtn) {
    				SendTojsp(JSONparam, "M838S020900E113");
    			} else{
    			       return false;
    			}
    	...
    	function SendTojsp(bomdata, pid) {
    	    $.ajax({
    			...
    			success: function (html) {	
    				if(html > -1) {
    					heneSwal.success('점검 기록 삭제가 완료되었습니다');

    					$('#modalReport').modal('hide');
    					parent.fn_MainInfo_List(startDate, endDate);
    	        		parent.fn_DetailInfo_List();
    	         	} else {
    					heneSwal.error('점검 기록 삭제에 실패했습니다, 다시 시도해주세요');	         		
    	         	}
    	         }
    	     });
    	}
