...
%>
 ...
 <script type="text/javascript">
    ...
    function fn_MainInfo_List(startDate, endDate) {
        $.ajax({
            type: "POST",
            url: "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S090200.jsp",
            data: "From=" + startDate + 
            	  "&To=" + endDate,
    ...
    function displayChecklist(element) {
    	...
    	if(seq_no.length < 1){
    		heneSwal.warning('조회할 고객 불만 및 처리 기록을 선택하세요');
    		return false;
    	}
    ...
    function haccpInsert(obj) {
    	var url = "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S090201.jsp"
    				+ '?checklist_id=' + checklist_id 
    				+ '&checklist_rev_no=' + checklist_rev_no;
    ...
    function haccpUpdate(obj) {
    	...
    	var url = "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S090202.jsp"
    				+ '?checklist_id=' + checklist_id 
    				+ '&checklist_rev_no=' + checklist_rev_no
    				+ '&seq_no=' + seq_no;
		var footer = '<button id="btn_Save" class="btn btn-info" onclick="SendTojsp();">저장</button>';
    ...
    function haccpDelete(obj) {
    	...
    	function SaveOderInfo() {
    			...
    			if(chekrtn) {
    				SendTojsp(JSONparam, "M838S090200E103");
    			} else{
    			       return false;
    			}
