...
%>
 ...
 <script type="text/javascript">
 	var checklist_id = "checklist21";		
	var checklist_rev_no = "";
	var checkup_date = "";
	var next_checkup_date = "";
    ...
    $(document).ready(function () {
		...
		$("#InfoContentTitle").html("건강진단 관리대장");
    ...
    function fn_MainInfo_List(startDate, endDate) {
        $.ajax({
            type: "POST",
            url: "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S080400.jsp",
            data: "From=" + startDate + 
            	  "&To=" + endDate,
    ...
    function displayChecklist(element) {
    	...
    	if(checkup_date.length < 1){
    		heneSwal.warning('조회할 건강진단 관리대장을 선택하세요');
    		return false;
    	}
    	...
    	jObj.checklistDate = checkup_date;
    ...
    function haccpInsert(obj) {
    	var url = "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S080401.jsp"
    				+ '?checklist_id=' + checklist_id 
    				+ '&checklist_rev_no=' + checklist_rev_no;
    ...
    function haccpUpdate(obj) {
    	...
    	if(checkup_date.length < 1){
    		heneSwal.warning('수정할 건강진단 관리대장을 선택하세요');
    		return false;
    	}
    	
    	var url = "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S080402.jsp"
    				+ '?checklist_id=' + checklist_id 
					+ '&checklist_rev_no=' + checklist_rev_no
					+ '&checkup_date=' + checkup_date					
					+ '&seq_no=' + seq_no
					+ '&regist_seq_no=' + regist_seq_no 
					+ '&checkup_nm=' + checkup_nm
					+ '&user_id=' + user_id
    ...
    function haccpDelete(obj) {
    	...
    	if(checkup_date.length < 1){
    		heneSwal.warning('삭제할 건강진단 관리대장을 선택하세요');
    		return false;
    	} else{
    		SaveOderInfo();
    	}
    	...
    	function SaveOderInfo() {
    			...
    			dataJson.checkup_date 	 	= checkup_date;
    			...
    			if(chekrtn) {
    				SendTojsp(JSONparam, "M838S080400E103");
    			} else{
    			       return false;
    			}
    	...
    	function SendTojsp(bomdata, pid) {
    	    $.ajax({
    			...
    			success: function (html) {	
    				if(html > -1) {
    					heneSwal.success('건강진단 관리대장 삭제가 완료되었습니다');

    					$('#modalReport').modal('hide');
    	        		parent.fn_MainInfo_List(startDate, endDate);
    	         	} else {
    					heneSwal.error('건강진단 관리대장 삭제에 실패했습니다, 다시 시도해주세요');	         		
    	         	}
    	         }
    	     });
    	}
