...
%>
 ...
 <script type="text/javascript">
    ...
    function fn_MainInfo_List(startDate, endDate) {
        $.ajax({
            type: "POST",
            url: "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S020200.jsp",
            data: "From=" + startDate + 
            	  "&To=" + endDate,
    ...
    function haccpInsert(obj) {
    	var url = "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S020201.jsp"
    ...
    function haccpUpdate(obj) {
    	...
    	var url = "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S020202.jsp"
			+ '?checklist_id=' + checklist_id
			+ '&checklist_rev_no=' + checklist_rev_no
    ...
    function haccpDelete(obj) {
    	...
    	if(check_date.length < 1){
    		heneSwal.warning('삭제할 점검일지를 선택하세요');
    		return false;
    	} else{
    		SaveOderInfo();
    	}
    	...
    	function SaveOderInfo() {
   			...
   			if(chekrtn) {
   				SendTojsp(JSONparam, "M838S020200E103");
   			} else {
   				return false;
   			}
    	}
    	...
    	function SendTojsp(bomdata, pid) {
    	    $.ajax({
    			...
    			success: function (html) {	
    				if(html > -1) {
    					heneSwal.success('점검일지 삭제가 완료되었습니다');

    					$('#modalReport').modal('hide');
    	        		parent.fn_MainInfo_List(startDate, endDate);
    	        		parent.$('#SubInfo_List_contents').hide();
    	         	} else {
    					heneSwal.error('점검일지 삭제에 실패했습니다, 다시 시도해주세요');	         		
    	         	}
    	         }
    	     });
    	}
