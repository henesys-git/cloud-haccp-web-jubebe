...
/* 	var approve_date = "";
    ...
    $(document).ready(function () {
		...
		$("#InfoContentTitle").html("부적합품 기록부");
    ...
    function fn_MainInfo_List(startDate, endDate) {
        $.ajax({
            type: "POST",
            url: "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S070650.jsp",
            data: "From=" + startDate + 
            	  "&To=" + endDate,
    ...
    function fn_DetailInfo_List() {     
    	$.ajax({
   	    	type: "POST",
   	        url: "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S070660.jsp",
    ...
    function haccpInsert(obj) {
    	var url = "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S070651.jsp"
    				+ '?checklist_id=' + checklist_id
    				+ '&checklist_rev_no=' + checklist_rev_no;
		var footer = '<button id="btn_Save" class="btn btn-info" onclick="inputNullCheck();">저장</button>';
		var title = obj.innerText;
		var heneModal = new HenesysModal(url, 'auto', title, footer);
		heneModal.open_modal();
    }
    ...
    function haccpUpdate(obj) {
    	...
    	if(regist_date.length < 1){
    		heneSwal.warning('수정할 기록목록을 선택하세요');
    		return false;
    	}
    	
    	var url = "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S070652.jsp"
			+ '?regist_seq_no=' + regist_seq_no;
    ...
    function haccpDelete(obj) {
    	...
    	if(regist_date.length < 1){
    		heneSwal.warning('삭제할 기록목록을 선택하세요');
    		return false;
    	} else{
    		SaveOderInfo();
    	}
    	...
    	function SaveOderInfo() {
   			...
   			if(chekrtn) {
   				SendTojsp(JSONparam, "M838S070650E103");
   			} else {
   				return false;
   			}
    	}
    	...
    	function SendTojsp(bomdata, pid) {
    	    $.ajax({
    			...
    			success: function (html) {	
    				if(html > -1) {
    					heneSwal.success('기록표 삭제가 완료되었습니다');

    					$('#modalReport').modal('hide');
    	        		parent.fn_MainInfo_List(startDate, endDate);
    	        		parent.$('#SubInfo_List_contents').hide();
    	         	} else {
    					heneSwal.error('기록표 삭제에 실패했습니다, 다시 시도해주세요');	         		
    	         	}
    	         }
    	     });
    	}
	...
	<div class="content-header">
  		<div class="container-fluid">
    		<div class="row mb-2">
	      		<div class="col-sm-6">
	        		<h1 class="m-0 text-dark" id="MenuTitle">부적합품 기록부</h1>
	      		</div>
	      		<div class="col-sm-6">
    	...
    	<div class="card-body">
    		<ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist">
	       		<li class="nav-item" onclick='fn_DetailInfo_List()'>
	       			<a class="nav-link" id="DetailInfo_List" data-toggle="pill" href="#SubInfo_List_contents" role="tab">원인 및 대책</a>
	       		</li>
	       		<li class="nav-item" onclick='fn_DetailInfo_List2()'>
	       			<a class="nav-link" id="DetailInfo_List2" data-toggle="pill" href="#SubInfo_List_contents2" role="tab2">조치 및 확인</a>
	       		</li>
	        </ul>
