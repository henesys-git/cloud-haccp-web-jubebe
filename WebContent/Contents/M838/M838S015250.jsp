...
%>
 ...
 <script type="text/javascript">
    ...
    $(document).ready(function () {
		...
		$("#InfoContentTitle").html("중요관리점(CCP) 검증점검 결과");
    ...
    function fn_MainInfo_List(startDate, endDate) {
        ...
        {
            type: "POST",
            url: "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S015250.jsp",
            data: "From=" + from + "&To=" + to + "&check_gubun=" + "<%=GV_GET_NUM_PREFIX%>",
            beforeSend: function () {
                $("#MainInfo_List_contents").children().remove();
            },
    ...
    function fn_HACCP_View_Canvas(obj) {
    	var vCheckDate	= $(obj).parent().parent().find("td").eq(0).text().trim();
    	var modalContentUrl  = "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S015250_canvas_tyens.jsp" 
							 + "?page_start=" + 1  // 1페이지	
    						 + "&check_gubun=" + "<%=GV_GET_NUM_PREFIX%>"
    						 + "&check_date=" + vCheckDate;
    	
    	pop_fn_popUpScr(modalContentUrl, "<%=sMenuTitle%>"+"(S838S070100_canvas)", '800px', '1500px');
    	return false;
	}
    ...
    function fn_DetailInfo_List() {    	
		...
		{
			type: "POST",
			url: "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S015260.jsp",
...
// 				$("#SubInfo_List_contents").children().remove();
 	...
 	// 원자재 입고검사 대장 등록(팝업창)
    function pop_fn_IMPORT_Check_Insert(obj) {
		var pageUrl = "<%=Config.this_SERVER_path%>/Contents/Business/M838/S838S015251.jsp";
    	var paramData = "check_gubun=" + "<%=GV_GET_NUM_PREFIX%>";
    	var pageTitle = obj.innerText+"(S838S070101)";
        ...
        <div class="panel-body">
            <div>
	            <button data-author="mainBtn" type="button"  id="mainButton"  class="btn btn-primary active"
	            			style="width: auto; float: left; "></button>
	            <button data-author="insert" type="button" onclick="pop_fn_IMPORT_Check_Insert(this)" id="insert" class="btn btn-default" 
	            			style="width: auto;float: left; margin-left:30px;">중요관리점(CCP) 검증점검 등록</button>
