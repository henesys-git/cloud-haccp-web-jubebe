...
public class ThermalServer extends Thread {
	...
	private String tempIpAddr = InterfaceConstants.TEMPERATURE_IP1;
	private int tempPortNo = InterfaceConstants.TEMPERATURE_PORT1;
		
    public ThermalServer() {
    	try {
    		System.out.println("[온도 센서] " + tempIpAddr);
    ...
    private void sendCommand2TempDev(int tempCodeInt, String commandGubun) {
		try {
			...
			//System.out.println(strToHf2211);
			...
			if(resultHex[0].equals("06")) { // if 06, received successfully
				char[] resultAscii = hexToAscii(resultHex);
				finalTemp = makeTemp(resultAscii);
				finalTemp = finalTemp.trim();
			}
			
			System.out.println("[온도 센서] "+ tempCodeInt + "번 온도계 온도 = " + finalTemp);
			...
			System.out.println("[온도 센서] 응답코드 : " + servletConnection.getResponseCode());
		} catch (Exception e) {
			e.printStackTrace();
		}
    }
    ...
    public void run() {
    	try {
    		while(true) {
    			...
    			System.out.println("[온도 센서] 조회 완료");
    ...
    public static void main(String args[]) {
		try {
	    	ThermalServer th = new ThermalServer();
	    	th.start();
		} catch (Exception e) {
			e.printStackTrace();
		}
