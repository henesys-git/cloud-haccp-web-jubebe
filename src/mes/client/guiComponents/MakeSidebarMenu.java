...
public class MakeSidebarMenu {
	...
	private String makeHeadMenu() {
		...
		String [] IconName = new String[11]; //AdminLte icon class 변수 배열 선언 및 정의
		...
		if(RowCount > 0) {
	        for(RowNum=0; RowNum < RowCount; RowNum++){ // menu_level 0(헤드메뉴) 구성     	
				if (meModel.getValueAt(RowNum,3).toString().equals("1") && meModel.getValueAt(RowNum,8).toString().equals("0")) {	//autho_menu=1, menu_level=0=> Head Menu
					...
					//html.append("		<i class='nav-icon fas fa-chart-pie'></i>"); //전체 동일한 icon class로 주입할때
					html.append("		<i class='"+IconName[MenuRowLimit]+"'></i>"); //배열 0-10번까지 icon class 주입 
					html.append("		<p>");
					html.append( 			paneNameKor );
					html.append("			<i class='right fas fa-angle-left'></i>");
					html.append("		</p>");
					html.append("	</a>");
					html.append(	makeSubMenu(packageName, paneNameKor, menuID));	// Sub menu 구성
					html.append("</li>");
		...
		for (SubRowNum = 0; SubRowNum < RowCount; SubRowNum++) {
			...
			if (meModel.getValueAt(SubRowNum, 9).toString().equals(ParentMenuID)) { // parrnt_menu_id 검사
				if (meModel.getValueAt(SubRowNum, 4).toString().equals("1") || // autho_insert
					...
					if (Integer.parseInt(meModel.getValueAt(SubRowNum, 8).toString()) == 1) { // menu_level 1이 있으면, Lv1
																								// 표시
						...
						Subhtml.append("	</ul>");
						Subhtml.append("</li>");
					} else if (Integer.parseInt(meModel.getValueAt(SubRowNum, 8).toString()) == 2) { // menu_level 1이
																										// 없으면, Sub
																										// Menu(Lv1)부분에
																										// Lv2를 표시
...
//	========================================= 밑에는 안 씀 =========================================
	...
	private String makeHeadMenuTablet() {
		...
		if (RowCount > 0) {
			...
			int nVertical = (int) Math.ceil((double) headMenuRowCount / nHorizon); // 세로 칸 개수 -> 총 개수에서 가로 칸 개수 나눈 후 소수점
	...
	private void makeSubSubMenuTablet(String HeadmenuID, String HeadmenuName, String SubMenuID, String SubmenuName) { // menu_level
		...
		int nVertical = (int) Math.ceil((double) subSubMenuRowCount / nHorizon); // 세로 칸 개수 -> 총 개수에서 가로 칸 개수 나눈 후 소수점
																					// 올림
		// 서브서브메뉴(Lv.2)구성
