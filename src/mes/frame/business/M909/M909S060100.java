...
public class M909S060100 extends SqlAdapter{
	Connection con = null;
	...
	public int doExcute(InoutParameter ioParam){
	    ...
	    try{
			...
			Method method = M909S060100.class.getMethod(event,optClass);
			LoggingWriter.setLogDebug(M909S060100.class.getName(),"==== " + event + " EventMethod Create Success ====");

			obj = method.invoke(M909S060100.class.newInstance(),optObj);
			doExcute_result = CommonFunction.getInt(obj.toString());
			
	    } catch (Exception ex) {
	    	doExcute_result = EventDefine.E_DOEXCUTE_INIT;
			ioParam.setMessage(MessageDefine.M_NO_EVENTID);
			LoggingWriter.setLogError(M909S060100.class.getName(),"==== EventID Not define : EVENT ID [ " + ioParam.getEventID() + "]");
	    } finally {
	    	obj = null;
			optClass = null;
			optObj = null;
		...
		LoggingWriter.setLogAll(M909S060100.class.getName(),"==== Query [수행시간  : " + runningTime + " ms]");
	...
	public int E101(InoutParameter ioParam){
		...
		try {
			...
			String sql = new StringBuilder()
					...
					/* .append("		'"+jObj.get("prod_gubun_s")+"',\n") */
					.append("		'"+jObj.get("start_date")+"',		\n")
					.append("		SYSDATETIME,							\n")
					.append("		'"+jObj.get("login_id")+"',			\n")
					.append("		'"+jObj.get("expiration_date")+"',	\n")
					.append("		'"+jObj.get("safe_stock")+"',		\n")
					.append("		'"+jObj.get("packing_cost")+"',		\n")
					//.append("		'"+jObj.get("unit_price")+"',		\n")
					//.append("		'"+jObj.get("proper_freeze_time")+"',\n")
	...
	public int E102(InoutParameter ioParam){
		...
		try {
			...
			String sql = new StringBuilder()
					...
					.append("       '"+jObj.get("member_key")+"' AS member_key \n")
					.append(") st\n")
					...
					.append("		tt.member_key = st.member_key \n")
			...
			sql = new StringBuilder()
					...
					.append("	modify_user_id = '"+jObj.get("login_id")+"', \n")
	...
	public int E105(InoutParameter ioParam){
		...
		try {
			...
			String sql = new StringBuilder()
					...
					.append("	AND delyn = 'N'														\n")
					.append("   AND P.revision_no = (SELECT MAX(revision_no) FROM tbm_product W		\n")
					.append("   				   WHERE P.prod_cd = W.prod_cd)						\n")
					.append("ORDER BY prod_cd ASC, revision_no DESC									\n")
					.toString();
	...
	public int E214(InoutParameter ioParam){
		...
		try {
			...
			String sql = new StringBuilder()
					...
					.append("FROM				\n")
					.append("	tbi_chulha A	\n")
					.append("INNER JOIN tbi_chulha_detail A2 		  \n")
					.append("ON A.chulha_no = A2.chulha_no 		      \n")
					.append("AND A2.chulha_rev_no = A2.chulha_rev_no  \n")
			...
			*/
			...
			String sql = new StringBuilder()
					...
					.append("AND B.cust_rev_no = C.revision_no   \n")
					.append("WHERE A.delyn = 'N'				 \n")
					.append("ORDER BY A.chulha_date DESC, C.cust_nm	ASC \n")
					.toString();
