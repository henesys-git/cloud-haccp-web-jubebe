 ...
 * 개요 : 이벤트ID별 메소드 정의
 */
public  class M404S010600 extends SqlAdapter{
	Connection con = null;
	...
	public int doExcute(InoutParameter ioParam){
	    ...
	    try{
			...
			Method method = M404S010600.class.getMethod(event,optClass);
			LoggingWriter.setLogDebug(M404S010600.class.getName(),"==== " + event + " EventMethod Create Success ====");

			obj = method.invoke(M404S010600.class.newInstance(),optObj);
			doExcute_result = CommonFunction.getInt(obj.toString());
			
	    } catch (Exception ex) {
	    	doExcute_result = EventDefine.E_DOEXCUTE_INIT;
			ioParam.setMessage(MessageDefine.M_NO_EVENTID);
			LoggingWriter.setLogError(M404S010600.class.getName(),"==== EventID Not define : EVENT ID [ " + ioParam.getEventID() + "]");
	    } finally {
	    	obj = null;
			optClass = null;
			optObj = null;
		...
		LoggingWriter.setLogAll(M404S010600.class.getName(),"==== Query [수행시간  : " + runningTime + " ms]");
	...
	public int E104(InoutParameter ioParam){
		...
		try {
			...
			e.printStackTrace();
			LoggingWriter.setLogError("M404S010600E104()","==== SQL ERROR ===="+ e.getMessage());
	    	...
	    	if (Config.useDataSource) {
				try {
					...
					LoggingWriter.setLogError("M404S010600E104()","==== finally ===="+ e.getMessage());
				}
	    	} else {
	    	}
	    }
		...
		try {
			...
			if(checklist_id.equals("checklist11")) {
				sql = new StringBuilder()
						...
						.append("AND (A.car_clean = 'X' OR A.visual_color = 'X' OR A.visual_smell = 'X' OR A.etc = 'X' OR docs_yn = 'X')  \n")
			...
			e.printStackTrace();
			LoggingWriter.setLogError("M404S010600E114()","==== SQL ERROR ===="+ e.getMessage());
			return EventDefine.E_DOEXCUTE_ERROR ;
	    } finally {
	    	if (Config.useDataSource) {
				try {
					...
					LoggingWriter.setLogError("M404S010600E114()","==== finally ===="+ e.getMessage());
				}
	    	}
	    }
