...
public class M505S260100 extends SqlAdapter{
	Connection con = null;
	...
	public  int doExcute(InoutParameter ioParam){
	    ...
	    try{
			...
			Method method = M505S260100.class.getMethod(event,optClass);
			LoggingWriter.setLogDebug(M505S260100.class.getName(),"==== " + event + " EventMethod Create Success ====");

			obj = method.invoke(M505S260100.class.newInstance(),optObj);
			doExcute_result = CommonFunction.getInt(obj.toString());
			
	    } catch (Exception ex) {
	    	doExcute_result = EventDefine.E_DOEXCUTE_INIT;
			ioParam.setMessage(MessageDefine.M_NO_EVENTID);
			LoggingWriter.setLogError(M505S260100.class.getName(),"==== EventID Not define : EVENT ID [ " + ioParam.getEventID() + "]");
	    } finally {
	    	obj = null;
			optClass = null;
			optObj = null;
		...
		LoggingWriter.setLogAll(M505S260100.class.getName(),"==== Query [수행시간  : " + runningTime + " ms]");
	...
	public int E111(InoutParameter ioParam){
		...
		try {
			...
			LoggingWriter.setLogError("M505S260100E111()","==== SQL ERROR ===="+ e.getMessage() + "\n" + sql.toString());
	    	...
	    	if (Config.useDataSource) {
				try {
					...
					LoggingWriter.setLogError("M505S260100E111()","==== finally ===="+ e.getMessage());
				}
	    	} else {
	    	}
	    }
	...
	public int E112(InoutParameter ioParam){
		...
		try {
			...
			LoggingWriter.setLogError("M505S260100E112()","==== SQL ERROR ===="+ e.getMessage());
			return EventDefine.E_DOEXCUTE_ERROR ;
	    } finally {
	    	if (Config.useDataSource) {
				try {
					...
					LoggingWriter.setLogError("M505S260100E112()","==== finally ===="+ e.getMessage());
				}
	    	} else {
	    	}
	    }
	...
	public int E113(InoutParameter ioParam){
		...
		try {
			...
			LoggingWriter.setLogError("M505S260100E113()","==== SQL ERROR ===="+ e.getMessage());
			return EventDefine.E_DOEXCUTE_ERROR ;
	    } finally {
	    	if (Config.useDataSource) {
				try {
					...
					LoggingWriter.setLogError("M505S260100E113()","==== finally ===="+ e.getMessage());
				}
	    	} else {
	    	}
	    }
...
//			String[][] c_paramArray = CommonFunction.getConvString2DoubleArray(rcvData);
			...
			sql.append(" where  sulbi_gubun = 'SELB002' 	\n"); // 생산설비목록 - 설비구분:SLB001
			sql.append("   AND  member_key = '" + jArray.get("member_key") + "' \n");
			...
			e.printStackTrace();
			LoggingWriter.setLogError("M505S260100E104()","==== SQL ERROR ===="+ e.getMessage());
			return EventDefine.E_DOEXCUTE_ERROR ;
	    } finally {
	    	if (Config.useDataSource) {
				try {
					...
					LoggingWriter.setLogError("M505S260100E104()","==== finally ===="+ e.getMessage());
				}
	    	} else {
	    	}
	    }
...
//			String[][] c_paramArray = CommonFunction.getConvString2DoubleArray(rcvData);
			...
			e.printStackTrace();
			LoggingWriter.setLogError("M505S260100E114()","==== SQL ERROR ===="+ e.getMessage());
			return EventDefine.E_DOEXCUTE_ERROR ;
	    } finally {
	    	if (Config.useDataSource) {
				try {
					...
					LoggingWriter.setLogError("M505S260100E114()","==== finally ===="+ e.getMessage());
				}
	    	} else {
	    	}
	    }
...
//			String[][] c_paramArray = CommonFunction.getConvString2DoubleArray(rcvData);
			...
			e.printStackTrace();
			LoggingWriter.setLogError("M505S260100E214()","==== SQL ERROR ===="+ e.getMessage());
			return EventDefine.E_DOEXCUTE_ERROR ;
	    } finally {
	    	if (Config.useDataSource) {
				try {
					...
					LoggingWriter.setLogError("M505S260100E214()","==== finally ===="+ e.getMessage());
				} 
	    	} else {
	    	}
	    }
